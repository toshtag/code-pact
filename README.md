...
`code-pact` gives any supported agent the same deterministic CLI for fetching task context, recording progress, and verifying completion criteria. The agent calls a small set of commands; `code-pact` currently keeps **active** plans in `design/` (roadmap, active phase YAML, decisions, rules) and records operational progress under `.code-pact/state/events/`. The project is transitioning **completed** phases and **retired** decisions toward `.code-pact/state` snapshots so historical design docs can be retired — see [the design-docs-ephemeral directive](design/decisions/design-docs-ephemeral-directive.md). Adapters generate the per-agent instruction files so each agent sees its own world without the project state diverging.
