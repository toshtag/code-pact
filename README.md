...
`code-pact` gives any supported agent the same deterministic CLI for fetching task context, recording progress, and verifying completion criteria. The agent calls a small set of commands; `code-pact` keeps `design/` as the structured source of truth and the progress ledger (per-event files under `.code-pact/state/events/`) as the operational log. Today, `design/` remains the source of truth for **active** plans; the project is transitioning **completed** phases and **retired** decisions toward `.code-pact/state` snapshots so historical design docs can be retired — see [the design-docs-ephemeral directive](design/decisions/design-docs-ephemeral-directive.md). Adapters generate the per-agent instruction files so each agent sees its own world without the project state diverging.
