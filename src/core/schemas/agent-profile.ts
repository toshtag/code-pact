...
export const AgentProfile = z.object({
  ...
  // Optional: pin the primary Claude model version. Read by `--model` validation,
  // profile persistence, the doctor model checks, and task recommendation — not by
  // the template: schema-v2 CLAUDE.md generation is model-neutral, so this pin
  // does not change the generated bytes.
  // Supported values: see CLAUDE_MODEL_VERSIONS in core/models/catalog.ts
  // (e.g. "opus-5" | "sonnet-5" | "opus-4.8" | "sonnet-4.6").
  // Omit to leave the profile unpinned.
