...
import {
  DeleteIntent,
  DELETE_INTENT_SCHEMA_VERSION,
  type BundlePairIntent,
  type DeleteIntentRecord,
} from "../schemas/delete-intent.ts";
import {
...
export class PendingDeleteIntentError extends Error {
  ...
  constructor() {
    super(
      "a delete-intent journal already exists; run recoverPendingDeletes before starting a new pair delete",
    );
...
let dirFsyncOverride:
  | ((dir: string, purpose: string) => Promise<void> | void)
  | null = null;
export function __setDeleteIntentDirFsyncForTests(
  fn: ((dir: string, purpose: string) => Promise<void> | void) | null,
): void {
  dirFsyncOverride = fn;
}
...
let fileFsyncOverride: ((purpose: string) => Promise<void> | void) | null =
  null;
export function __setDeleteIntentFileFsyncForTests(
  fn: ((purpose: string) => Promise<void> | void) | null,
): void {
...
export async function fsyncFileRequired(
  ...
  try {
    ...
    throw new DeleteIntentDurabilityError(
      "failed",
      `file fsync failed (${purpose}): ${(err as Error).message}`,
    );
  }
}
...
export async function writeDeleteIntent(
  cwd: string,
  intents: DeleteIntentRecord[],
): Promise<void> {
...
export async function readDeleteIntent(cwd: string): Promise<DeleteIntentRead> {
  ...
  try {
    ...
    return {
      kind: "corrupt",
      cause: "io_error",
      detail: `delete-intent journal unreadable: ${(err as Error).message}`,
    };
  }
  ...
  try {
    ...
    return {
      kind: "corrupt",
      cause: "parse_error",
      detail: `delete-intent journal is not a valid intent: ${(err as Error).message}`,
    };
  }
  ...
  if (raw !== serializeDeleteIntent(intent)) {
    return {
      kind: "corrupt",
      cause: "parse_error",
      detail: "delete-intent journal is not the writer's canonical bytes",
    };
  }
  ...
  if (new Set(ids).size !== ids.length) {
    return {
      kind: "corrupt",
      cause: "parse_error",
      detail: "delete-intent journal names a phase_id more than once",
    };
  }
    ...
    for (const kind of ["phase_snapshot", "event_pack"] as const) {
      ...
      if (m.removed_ids.length !== 1 || m.removed_ids[0] !== i.phase_id) {
        return {
          kind: "corrupt",
          cause: "parse_error",
          detail: `bundle_pair ${i.phase_id} ${kind}.removed_ids must be exactly [${i.phase_id}]`,
        };
      }
      const files = [
        ...m.old_bundles.map(b => b.file),
        ...(m.new_bundle ? [m.new_bundle.file] : []),
      ];
      if (files.some(f => !f.startsWith(`${kind}-`))) {
        return {
          kind: "corrupt",
          cause: "parse_error",
          detail: `bundle_pair ${i.phase_id} ${kind} names a bundle file of another kind`,
        };
      }
    }
  }
...
export async function readPendingDeleteIds(
  cwd: string,
  ...
  return new Set(
    read.intent.intents
      .filter(i => i.intent_kind === "loose_pair")
      .map(i => i.phase_id),
  );
}
 ...
 *  READ-ONLY. */
export async function readPendingBundleDeleteIds(
  cwd: string,
): Promise<ReadonlySet<string>> {
...
export async function readPendingDeleteFilters(
  cwd: string,
): Promise<PendingDeleteFilters> {
  const read = await readDeleteIntent(cwd);
  if (read.kind !== "present")
    return { looseAbsentIds: new Set(), bundleAbsentIds: new Set() };
  ...
  for (const intent of read.intent.intents) {
    if (intent.intent_kind === "loose_pair") {
      ...
      for (const id of intent.members.phase_snapshot.removed_ids)
        bundleAbsentIds.add(id);
      for (const id of intent.members.event_pack.removed_ids)
        bundleAbsentIds.add(id);
    }
  }
...
async function completeLoosePairUnlinks(
  cwd: string,
  phaseIds: string[],
  ...
  for (const phaseId of phaseIds) {
    ...
    ); // pack first; either order is healed by recovery
    if (hooks.afterPackUnlinked) await hooks.afterPackUnlinked(phaseId);
    await unlinkIfPresent(
      await resolveArchiveOwnedPath(cwd, phaseSnapshotRelPath(phaseId)),
    );
...
export async function completePairsThenClear(
  cwd: string,
  phaseIds: string[],
  hooks: PairUnlinkHooks = {},
): Promise<void> {
...
export async function assertBundlePairsCommittable(
  cwd: string,
  pairs: BundlePairIntent[],
): Promise<void> {
  const readMatch = async (
    ...
    try {
      ...
      throw new BundlePairNotCommittableError(
        `${what} ${file} is missing before commit: ${(err as Error).message}`,
      );
    }
    if (sha256Hex(raw) !== expected) {
      throw new BundlePairNotCommittableError(
        `${what} ${file} changed before commit (digest no longer matches the plan)`,
      );
    }
  };
  for (const pair of pairs) {
    for (const kind of ["phase_snapshot", "event_pack"] as const) {
      ...
      if (member.new_bundle != null)
        await readMatch(
          member.new_bundle.file,
          member.new_bundle.sha256,
          `survivor bundle (${kind}, ${pair.phase_id})`,
        );
      for (const old of member.old_bundles)
        await readMatch(
          old.file,
          old.sha256,
          `old bundle (${kind}, ${pair.phase_id})`,
        );
    }
  }
}
...
async function completeBundlePairRetires(
  cwd: string,
  pairs: BundlePairIntent[],
  hooks: BundlePairRetireHooks = {},
): Promise<void> {
  ...
  for (const pair of pairs) {
    for (const kind of ["phase_snapshot", "event_pack"] as const) {
      ...
      if (member.new_bundle != null) {
        const newPath = await resolveArchiveBundleFile(
          cwd,
          member.new_bundle.file,
        );
        let newRaw: string;
        ...
        if (sha256Hex(newRaw) !== member.new_bundle.sha256) {
          throw new DeleteIntentRecoveryError(
            `bundle-pair recovery: survivor bundle ${member.new_bundle.file} does not match the committed digest (${kind}, ${pair.phase_id})`,
          );
        }
      }
...
export async function completeBundlePairsThenClear(
  cwd: string,
  pairs: BundlePairIntent[],
  hooks: BundlePairRetireHooks = {},
): Promise<void> {
...
export async function recoverPendingDeletes(
  cwd: string,
): Promise<RecoveryOutcome> {
  const read = await readDeleteIntent(cwd);
  if (read.kind === "absent")
    return { completed: [], loose_pairs: [], bundle_pairs: [] };
  if (read.kind === "corrupt") throw new DeleteIntentRecoveryError(read.detail);
  ...
  for (const intent of read.intent.intents) {
    if (intent.intent_kind === "loose_pair")
      loosePhaseIds.push(intent.phase_id);
  ...
  return {
    completed: [...loosePhaseIds, ...bundlePhaseIds],
    loose_pairs: loosePhaseIds,
    bundle_pairs: bundlePhaseIds,
  };
}
