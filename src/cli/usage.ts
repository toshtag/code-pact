...
const CLUSTER_SUBCOMMANDS: Record<string, string> = {
  ...
  task: "add | context | prepare | start | status | block | resume | complete | record-done | finalize | runbook | cancel | execute | lock | review-bundle | ci-parity (aliases: reconcile = finalize, next = runbook)",
...
const LEAF_USAGE: Record<string, () => string> = {
  ...
  "task cancel": () => renderLeafHelp(TASK_SPECS.cancel!),
