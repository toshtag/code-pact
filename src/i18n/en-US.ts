    ...
    adapterCommon: {
      ...
      agentContract: {
        ...
        failBody: [
          ...
          "- **verification failure** (from `verify --json --detail agent` or `task complete --json --detail agent`) — `error.code` is `VERIFICATION_FAILED` (exit 1). Read `error.cause_code` first: `COMMANDS_FAILED` → fix the failing verification command; `DECISION_REQUIRED` → a `requires_decision` task needs an accepted ADR (write/accept it); `ABORTED` → retry only after the interruption is resolved.",
          "- For agent-detail verification failures, `error.message` is intentionally short. Diagnose in this order: `data.failure.kind`, `data.failure.fingerprint`, `data.failure.stderr_excerpt`, `data.failure.stdout_excerpt`, `data.failure.evidence_available`, `data.failure.evidence_error`, then `data.failure.retrieve_command`.",
          "- Do not retrieve full evidence by default. Use `data.failure.retrieve_command` only when the excerpts are insufficient to decide the fix.",
