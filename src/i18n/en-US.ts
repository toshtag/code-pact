  ...
  templates: {
    ...
    adapterCommon: {
      ...
      agentContract: {
        ...
        contextCommandBody:
          "Use `data.commands.context` exactly as returned by `task prepare`. Do not reconstruct, widen, or replace the resolved context budget. Budgeted context may contain deterministic structural projections. Use the projected form first. Retrieve an exact original section only when a specific missing detail blocks the task; do not retrieve every projected section by default.",
        whenBody: [
