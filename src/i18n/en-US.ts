    ...
    adapterCommon: {
      ...
      agentContract: {
        ...
        failBody: [
          ...
          "- **standalone verify failure** (from `verify --json --detail agent`) — `error.cause_code` is guaranteed only for cancellation (`ABORTED`). For ordinary failures, branch on `data.failure.kind`: `command_failed`, `timed_out`, or `decision_required`.",
