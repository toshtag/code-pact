...
export type DecisionRetireResult =
  | {
      ...
      decision: string;
      record_path: string;
      record_action: "write" | "refresh" | "noop";
    }
...
type Presence =
  | { kind: "present"; abs: OwnedReadPath }
  | { kind: "absent" }
...
async function classifyParent(parentAbs: OwnedListPath): Promise<Presence> {
  ...
  try {
    ...
    return {
      ...
      detail: isEnoent(err)
        ? "parent directory of the decision does not exist"
        : `lstat parent: ${(err as Error).message}`,
    };
  }
  if (pst.isSymbolicLink()) {
    return {
      ...
      detail: "decision's parent is a symlink (dangling or redirected)",
    };
  }
  if (!pst.isDirectory()) {
    return {
      kind: "inaccessible",
      reason: "path_inaccessible",
      detail: "decision's parent is not a directory",
    };
  }
  return { kind: "absent" };
}
...
async function decisionMdPresence(
  cwd: string,
  canonical: string,
): Promise<Presence> {
  ...
  if (st.isSymbolicLink()) {
    return {
      ...
      reason: "identity_changed",
      detail:
        "decision path is a symlink (dangling or not); refusing to retire through it",
    };
  }
  if (!st.isFile()) {
    return {
      ...
      detail: st.isDirectory()
        ? "decision path is a directory"
        : "decision path is not a regular file",
    };
  }
...
async function inspectDecisionMd(
  ...
  if (lst.isSymbolicLink())
    return {
      ok: false,
      reason: "identity_changed",
      detail: "decision path is a symlink",
    };
  if (!lst.isFile())
    return {
      ok: false,
      reason: "identity_changed",
      detail: "decision path is not a regular file",
    };
  let content: string;
  try {
  ...
  try {
    ...
    return {
      ok: false,
      reason: "path_inaccessible",
      detail: `stat: ${(err as Error).message}`,
    };
  }
  if (expected) {
    if (source_sha256 !== expected.source_sha256)
      return {
        ...
        detail: "decision bytes changed since the baseline",
      };
    if (st.ino !== expected.ino || st.dev !== expected.dev)
      return {
        ok: false,
        reason: "identity_changed",
        detail: "decision inode/dev changed (file swapped)",
      };
  }
...
> {
  ...
  const matched = recordMatchingRef(
...
export async function runDecisionRetire(
  opts: DecisionRetireOptions,
): Promise<DecisionRetireResult> {
  ...
  if (canonical === null) {
    return {
      ...
      blocks: [
        {
          gate: "target_invalid",
          detail: `"${rawPath}" is not a retireable decision (expected a .md decision record under design/decisions/)`,
        },
      ],
    };
  }
  ...
  if (presence.kind === "absent") {
    ...
    if (!rb.ok) {
      return {
        ...
        decision: canonical,
        reason:
          "the decision is gone and no valid decision-state record resolves it",
      };
    }
    return write
      ? {
  ...
  if (artifactDetail !== null)
    blocks.push({ gate: "plan_artifacts_unreadable", detail: artifactDetail });
  if (blocks.length > 0) {
  ...
  if (!write) {
    ...
    try {
      ...
      return {
        ...
        detail: `record plan failed: ${(err as Error).message}`,
      };
    }
    if (plan.kind === "ineligible") {
      return {
        ...
        decision: canonical,
        reason: "record_unverified",
        detail: blockDetail(plan.blocks),
      };
    }
    return {
      kind: "would_retire",
  ...
  try {
    ...
    return {
      ...
      decision: canonical,
      reason: "record_unverified",
      detail: `record write failed: ${(err as Error).message}`,
    };
  }
  if (outcome.kind === "ineligible") {
    return {
      ...
      decision: canonical,
      reason: "record_unverified",
      detail: blockDetail(outcome.blocks),
    };
  }
  const recordAction: "write" | "noop" =
    outcome.kind === "written" ? "write" : "noop";
  ...
  if (!rb.ok) {
    return {
      ...
      reason: "record_unverified",
      detail:
        "the written record is not reader-resolvable (identity check failed)",
    };
  }
  if (rb.source_sha256 !== baseline.source_sha256) {
    return {
      ...
      reason: "record_unverified",
      detail:
        "the written record's source_sha256 does not match the live decision bytes",
    };
  }
  ...
  const recheckPhases: PhaseEntry[] =
    recheckPlan.state?.phases ?? recheckPlan.fallbackPhases;
  const recheckArtifact = planArtifactsUnreadable(
    recheckPlan.fileIssues,
    recheckPlan.skippedChecks,
  );
  if (recheckArtifact !== null) {
    return {
      ...
      decision: canonical,
      reason: "path_inaccessible",
      detail: recheckArtifact,
    };
  }
  const recheckBlocks = await recheckRetireExternalState(
    cwd,
    canonical,
    recheckPhases,
    rb.may_satisfy,
  ...
  if (recheckBlocks.length > 0) {
    ...
    const hasTargetChange = recheckBlocks.some(
      b =>
        b.gate === "target_missing" ||
        b.gate === "target_unreadable" ||
        b.gate === "target_invalid",
    );
    const hasUnreadableScan = recheckBlocks.some(
      b =>
        b.gate === "dependency_unreadable" ||
        b.gate === "decision_scan_unreadable",
    );
    ...
    return {
      ...
      decision: canonical,
      reason,
      detail: retireBlockDetail(recheckBlocks),
    };
  }
...
function blockDetail(blocks: DecisionRecordBlock[]): string {
  return blocks
    .map(b => b.kind + ("detail" in b ? `: ${b.detail}` : ""))
    .join("; ");
}
