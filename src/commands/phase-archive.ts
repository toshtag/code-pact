...
export type PhaseArchiveResult =
  | {
      ...
      snapshot_action: "write" | "refresh" | "noop";
    }
  | {
      kind: "would_already_archived";
      phase_id: string;
      yaml_path: string;
      snapshot_path: string;
    }
  ...
  | {
      ...
      phase_id: string;
      yaml_path: string;
      reason: PhaseArchiveStaleReason;
      detail: string;
    };
...
async function classifyParent(parentAbs: OwnedListPath): Promise<Presence> {
  ...
  try {
    ...
    return {
      ...
      detail: isEnoent(err)
        ? "parent directory of the phase YAML does not exist"
  ...
  if (pst.isSymbolicLink()) {
    return {
      ...
      detail: "phase YAML's parent is a symlink (dangling or redirected)",
    };
  }
  if (!pst.isDirectory()) {
    return {
      kind: "inaccessible",
      reason: "path_inaccessible",
      detail: "phase YAML's parent is not a directory",
    };
  }
 ...
 */
async function phaseYamlPresence(
  cwd: string,
  relPath: string,
): Promise<Presence> {
  let abs: OwnedReadPath;
  try {
    abs = await resolvePhaseReadPath(cwd, relPath);
  } catch (err) {
    return {
      kind: "inaccessible",
      reason: "path_inaccessible",
  ...
  let st;
  try {
    st = await lstatOwned(abs); // does NOT follow the final symlink
  } catch (err) {
    if (isEnoent(err)) {
  ...
  if (st.isSymbolicLink()) {
    return {
      ...
      reason: "identity_changed",
      detail:
        "phase YAML path is a symlink (dangling or not); refusing to archive through it",
    };
  }
  if (!st.isFile()) {
    return {
      ...
      detail: st.isDirectory()
        ? "phase YAML path is a directory"
        : "phase YAML path is not a regular file",
    };
  }
...
async function inspectPhaseYaml(
  ...
  if (lst.isSymbolicLink()) {
    return {
      ok: false,
      reason: "identity_changed",
      detail: "phase YAML path is a symlink",
    };
  }
  if (!lst.isFile()) {
    return {
      ok: false,
      reason: "identity_changed",
      detail: "phase YAML path is not a regular file",
    };
  }
  let content: string;
  try {
  ...
  if (expected) {
    if (source_sha256 !== expected.source_sha256) {
      return {
        ...
        detail: "phase YAML bytes changed since the baseline",
      };
    }
    if (st.ino !== expected.ino || st.dev !== expected.dev) {
      return {
        ok: false,
        reason: "identity_changed",
        detail: "phase YAML inode/dev changed (file swapped)",
      };
    }
  }
...
async function readbackResolve(
  ...
  | { ok: true; snapshot_path: string; source_sha256: string }
...
> {
  ...
  if (res.kind === "tolerated") {
    ...
    return {
      ok: true,
      snapshot_path: phaseSnapshotPath(cwd, ref.id),
      source_sha256: res.snapshot.source_sha256,
    };
  }
...
export async function runPhaseArchive(
  opts: PhaseArchiveOptions,
): Promise<PhaseArchiveResult> {
  ...
  if (presence.kind === "absent") {
    ...
    return write
      ? {
          ...
          phase_id: ref.id,
          yaml_path: yamlPath,
          snapshot_path: rb.snapshot_path,
        }
      : {
          kind: "would_already_archived",
          phase_id: ref.id,
  ...
  if (!write) {
    const plan = await planPhaseSnapshot(cwd, phaseId, {
      now,
    ...
    const action =
      plan.kind === "noop_same_source" ||
      plan.kind === "noop_record_authoritative"
        ? "noop"
        : plan.kind;
    return {
      kind: "would_archive",
  ...
  const outcome = await writePhaseSnapshot(cwd, phaseId, {
    now,
    ...(attestations ? { attestations } : {}),
  });
  ...
  const snapshotAction = outcome.kind === "written" ? "write" : "noop"; // noop_same_source / noop_record_authoritative
  ...
  if (rb.source_sha256 !== baseline.source_sha256) {
    return {
      ...
      reason: "snapshot_unverified",
      detail:
        "tolerated snapshot source_sha256 does not match the live YAML bytes",
    };
  }
