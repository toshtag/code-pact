...
describe("adapter generation honors a custom profile path end-to-end", () => {
  it("reads and pins model_version to the project's profile path, not the default", async () => {
    ...
    // The instruction file was generated from that profile. It is a
    // model-neutral bootstrap, so the pin shows up in the profile only — the
    // generated bytes carry no model guidance.
    const claudeMd = await readFile(join(dir, "CLAUDE.md"), "utf8");
    expect(claudeMd).toContain("code-pact task prepare");
    expect(claudeMd).not.toContain("Model guidance");
  });
});
