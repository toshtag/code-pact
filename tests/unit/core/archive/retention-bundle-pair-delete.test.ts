...
import {
  ProgressLog,
  type ProgressEvent,
} from "../../../../src/core/schemas/progress-event.ts";
...
} from "../../../../src/core/schemas/archive-bundle.ts";
...
beforeEach(async () => {
  ...
  await mkdir(join(cwd, ".code-pact", "state", "archive", "phases"), {
    recursive: true,
  });
  await mkdir(join(cwd, ".code-pact", "state", "archive", "event-packs"), {
    recursive: true,
  });
});
afterEach(async () => {
...
async function pairBytes(id: string): Promise<{ snap: string; pack: string }> {
  ...
  expect((await writePhaseSnapshot(cwd, id, { now: NOW })).kind).toBe(
    "written",
  );
...
async function bundlePairs(
  ids: string[],
  bytesById: Map<string, { snap: string; pack: string }>,
): Promise<void> {
  await writeArchiveBundle(
    cwd,
    "phase_snapshot",
    ids.map(id => ({ id, bytes: bytesById.get(id)!.snap })),
  );
  await writeArchiveBundle(
    cwd,
    "event_pack",
    ids.map(id => ({ id, bytes: bytesById.get(id)!.pack })),
  );
}

async function listBundleNames(): Promise<string[]> {
  return readdir(archiveBundlesDir(cwd)).then(
    ns => ns.filter(n => n.endsWith(".json")).sort(),
    () => [],
  );
}
...
async function resolvesInBundles(
  id: string,
): Promise<{ phase: boolean; pack: boolean }> {
  const idx = loadArchiveBundles(cwd).index;
  return {
    phase: idx.get("phase_snapshot")?.has(id) ?? false,
    pack: idx.get("event_pack")?.has(id) ?? false,
  };
}
...
describe("deleteBundlePairsJournaled — both-or-neither bundle-pair removal", () => {
  it("removes a bundle pair: reduced bundles written, old retired, journal cleared, both sides deleted", async () => {
    ...
    expect(out.removed).toEqual([
      { phase_id: "P1", phase_snapshot: "deleted", event_pack: "deleted" },
    ]);
    expect(out.skipped).toEqual([]);
    // P1 is gone from BOTH bundle stores; P2/P3 survive.
    expect(await resolvesInBundles("P1")).toEqual({
      phase: false,
      pack: false,
    });
  ...
  it("a pair whose pack is NOT a bundle member is deferred whole (not_bundle_member), nothing removed", async () => {
    ...
    await writeArchiveBundle(
      cwd,
      "phase_snapshot",
      ["P1", "P2"].map(id => ({ id, bytes: bytesById.get(id)!.snap })),
    );

    const out = await deleteBundlePairsJournaled(cwd, [{ phase_id: "P1" }]);
    expect(out.removed).toEqual([]);
    expect(out.skipped).toEqual([
      { phase_id: "P1", reason: "not_bundle_member" },
    ]);
  ...
  it("a `both` pair (loose copies present) → bundle_member_removed (the loose halves survive)", async () => {
    ...
    await writeFile(
      phaseSnapshotPath(cwd, "P1"),
      bytesById.get("P1")!.snap,
      "utf8",
    );
    await writeFile(
      eventPackPath(cwd, "P1"),
      bytesById.get("P1")!.pack,
      "utf8",
    );
    ...
    expect(out.removed).toEqual([
      {
        phase_id: "P1",
        phase_snapshot: "bundle_member_removed",
        event_pack: "bundle_member_removed",
      },
    ]);
    // The loose halves still resolve (old truth remains until the loose layer drops it next run).
    expect(await readFile(phaseSnapshotPath(cwd, "P1"), "utf8")).toBe(
      bytesById.get("P1")!.snap,
    );
    expect(await readFile(eventPackPath(cwd, "P1"), "utf8")).toBe(
      bytesById.get("P1")!.pack,
    );
  });
  ...
  it("an authority-INVALID event_pack member (misfiled: id P1, body phase_id P2) → unsafe_authority, untouched", async () => {
    ...
    await writeArchiveBundle(cwd, "phase_snapshot", [
      { id: "P1", bytes: bytesById.get("P1")!.snap },
    ]);
    await rawBundle("event_pack", [
      { id: "P1", bytes: bytesById.get("P2")!.pack },
    ]);
    const before = await listBundleNames();
    ...
    expect(out.removed).toEqual([]);
    expect(out.skipped).toEqual([
      { phase_id: "P1", reason: "unsafe_authority" },
    ]); // the kind is fail-closed
    expect(await listBundleNames()).toEqual(before); // nothing written, nothing retired
    expect((await readDeleteIntent(cwd)).kind).toBe("absent"); // no commit
  });
  ...
  it("TWO pairs sharing the same bundles are removed consistently (consolidated survivor)", async () => {
    ...
    const out = await deleteBundlePairsJournaled(cwd, [
      { phase_id: "P1" },
      { phase_id: "P2" },
    ]);
    ...
    expect(await resolvesInBundles("P2")).toEqual({
      phase: false,
      pack: false,
    });
...
describe("deleteBundlePairsJournaled — crash safety (journal recovery)", () => {
  ...
  it("a crash BEFORE the commit (durable write fails) → no journal, both old bundles intact, a re-run completes", async () => {
    ...
    __setDeleteIntentDirFsyncForTests((_d, purpose) => {
      if (purpose === "bundle_write")
        throw new DeleteIntentDurabilityError("failed", "injected");
    });
    await expect(
      deleteBundlePairsJournaled(cwd, [{ phase_id: "P1" }]),
    ).rejects.toBeInstanceOf(DeleteIntentDurabilityError);
      ...
      deleteBundlePairsJournaled(cwd, [{ phase_id: "P1" }], {
        afterIntentWritten: async () => {
          ...
          for (const f of await listBundleNames()) {
            ...
            if (!raw.includes('"P1"'))
              await rm(join(archiveBundlesDir(cwd), f)); // the reduced survivor bundle
          }
          throw new Error("crash");
        },
      }),
    ).rejects.toThrow("crash");
    await expect(recoverPendingDeletes(cwd)).rejects.toBeInstanceOf(
      DeleteIntentRecoveryError,
    );
  });
    ...
    await expect(
      deleteBundlePairsJournaled(cwd, [{ phase_id: "P1" }], {
        beforeIntentWritten: async () => {
          ...
          for (const f of await listBundleNames()) {
            if (
              f.startsWith("phase_snapshot-") &&
              (
                await readFile(join(archiveBundlesDir(cwd), f), "utf8")
              ).includes('"P1"')
            ) {
              await writeFile(
                join(archiveBundlesDir(cwd), f),
                (await readFile(join(archiveBundlesDir(cwd), f), "utf8")) + " ",
                "utf8",
              );
            }
        ...
        beforeIntentWritten: async () => {
          ...
          for (const f of await listBundleNames()) {
            if (
              ...
              await rm(join(archiveBundlesDir(cwd), f));
          }
        },
      }),
    ).rejects.toBeInstanceOf(BundlePairNotCommittableError);
  ...
  it("an `unsupported` directory-fsync platform DEFERS every pair (no write, no retire)", async () => {
    ...
    const before = await listBundleNames();
    __setDeleteIntentDirFsyncForTests((_d, purpose) => {
      if (purpose === "bundle_removal_preflight")
        throw new DeleteIntentDurabilityError("unsupported", "injected");
    });
    const out = await deleteBundlePairsJournaled(cwd, [{ phase_id: "P1" }]);
    expect(out.removed).toEqual([]);
    expect(out.skipped).toEqual([
      { phase_id: "P1", reason: "unsupported_platform" },
    ]);
...
describe("deleteBundlePairsJournaled — reader-awareness in the pending window", () => {
  ...
  it("readDeleteIntent rejects a writer-impossible bundle_pair (removed_ids != [phase_id]) as corrupt", async () => {
    ...
    const j = (members: unknown): string =>
      JSON.stringify(
        {
          schema_version: 2,
          intents: [{ intent_kind: "bundle_pair", phase_id: "P1", members }],
        },
        null,
        2,
      ) + "\n";
    const ok = {
      ...
      old_bundles: [
        {
          file: "phase_snapshot-0123456789abcdef.json",
          sha256: sha256Hex("a"),
        },
      ],
      new_bundle: null,
    ...
    const okPack = {
      removed_ids: ["P1"],
      old_bundles: [
        { file: "event_pack-0123456789abcdef.json", sha256: sha256Hex("b") },
      ],
      new_bundle: null,
    };
    // removed_ids names a DIFFERENT id than the pair's phase_id → corrupt (a writer never emits this).
    await writeFile(
      join(cwd, ".code-pact", "state", "archive", "delete-intent.json"),
      j({ phase_snapshot: { ...ok, removed_ids: ["PX"] }, event_pack: okPack }),
      "utf8",
    );
  ...
  it("readDeleteIntent rejects a bundle_pair whose file names another kind's bundle as corrupt", async () => {
    ...
    const phaseMember = {
      removed_ids: ["P1"],
      old_bundles: [
        { file: "event_pack-0123456789abcdef.json", sha256: sha256Hex("a") },
      ],
      new_bundle: null,
    }; // wrong-kind prefix
    const packMember = {
          ...
          intents: [
            {
              ...
              phase_id: "P1",
              members: { phase_snapshot: phaseMember, event_pack: packMember },
            },
          ],
        },
        null,
    ...
    await expect(
      deleteBundlePairsJournaled(cwd, [{ phase_id: "P1" }], {
        afterIntentWritten: async () => {
          ...
          looseDuringWindow = {
            phase: (await enumerateLooseMembers(cwd, "phase_snapshot")).map(
              m => m.id,
            ),
            pack: (await enumerateLooseMembers(cwd, "event_pack")).map(
              m => m.id,
            ),
          };
          throw new Error("stop");
