...
import {
  deriveSkillName,
  deriveSkillNameVariants,
} from "../../../src/core/adapters/claude.ts";
import {
  writeManifest,
  computeContentHash,
...
describe("runGenerateAdapter — claude-code", () => {
  ...
  it("skips existing files when force is false", async () => {
    ...
    const second = await runGenerateAdapter({
      cwd: dir,
      agentName: "claude-code",
      force: false,
      locale: "en-US",
  ...
  it("install is fully idempotent — second run produces identical manifest hashes", async () => {
    const first = await runGenerateAdapter({
      cwd: dir,
      agentName: "claude-code",
      force: false,
      locale: "en-US",
...
describe("runGenerateAdapter — generic", () => {
  ...
  it("writes docs/code-pact/agent-instructions.md", async () => {
    ...
    const names = result.created.map(p => p.replace(dir, ""));
    expect(
      names.some(n => n.includes("docs/code-pact/agent-instructions.md")),
    ).toBe(true);
  });
...
describe("deriveSkillName", () => {
  ...
  it("pnpm test → test", () =>
    expect(deriveSkillName("pnpm test")).toBe("test"));
  it("pnpm typecheck → typecheck", () =>
    expect(deriveSkillName("pnpm typecheck")).toBe("typecheck"));
  it("pnpm build → build", () =>
    expect(deriveSkillName("pnpm build")).toBe("build"));
  it("npm run lint → lint", () =>
    expect(deriveSkillName("npm run lint")).toBe("lint"));
  it("yarn dev → dev", () => expect(deriveSkillName("yarn dev")).toBe("dev"));
  it("bun run test:unit → test-unit", () =>
    expect(deriveSkillName("bun run test:unit")).toBe("test-unit"));
  ...
  it("make build → make-build", () =>
    expect(deriveSkillName("make build")).toBe("make-build"));
  ...
  it("drops a space-separated flag value (adapter doctor --agent claude-code)", () =>
    expect(
      deriveSkillName("code-pact adapter doctor --agent claude-code"),
    ).toBe("adapter-doctor"));
  it("drops a --flag=value form (validate --json)", () =>
    expect(deriveSkillName("node dist/cli.js validate --json")).toBe(
      "validate",
    ));
  ...
  it("trailing flag with no value (adapter doctor --agent)", () =>
    expect(deriveSkillName("code-pact adapter doctor --agent")).toBe(
      "adapter-doctor",
    ));
  ...
  it("a flag does not consume a following word for naming (plan lint --strict extra)", () =>
    expect(deriveSkillName("code-pact plan lint --strict extra")).toBe(
      "plan-lint",
    ));
});
...
describe("deriveSkillNameVariants", () => {
  ...
  it("walks base → flag-qualified forms in order", () => {
    expect(
      deriveSkillNameVariants("code-pact adapter upgrade --check --json"),
    ).toEqual([
      "adapter-upgrade",
...
describe("runGenerateAdapter — v0.5.2 skill generation", () => {
  ...
  it("--regen-skills does NOT overwrite a divergent DYNAMIC skill outside the owned set (security)", async () => {
    ...
    // PRESERVED (warn), not overwritten — the shared namespace cannot prove
  ...
  it("multiple phases with the same command produce one skill file", async () => {
    ...
    const roadmapContent = await readFile(
