...
async function scaffold(): Promise<void> {
  ...
  await writeFile(
    join(tmpDir, "design", "decisions", "accepted-rfc.md"),
    ACCEPTED,
    "utf8",
  );
  await writeFile(
    join(tmpDir, "design", "decisions", "blocked-rfc.md"),
    BLOCKED,
    "utf8",
  );
...
describe("hand-deleting a completed phase + rm -rf design/decisions keeps all gates green", () => {
  it("archive + retire via the verbs, then a bare rm of the directory; every gate (incl. check:docs) stays green", async () => {
    ...
    expect(
      decisionCheckOk(
        run(["verify", "--phase", "P2", "--task", "P2-T1", "--json"]),
      ),
    ).toBe(true);
    ...
    expect(await readFile(join(tmpDir, "docs", "notes.md"), "utf8")).toBe(
      notesBefore,
    );
  }, 60_000);
});
