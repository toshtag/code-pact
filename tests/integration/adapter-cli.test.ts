...
beforeEach(async () => {
  ...
  dir = await realpath(
    await mkdtemp(join(tmpdir(), "code-pact-adapter-cli-test-")),
  );
  await runInit({
    cwd: dir,
    locale: "en-US",
...
describe("adapter upgrade — unowned orphan warn output (security)", () => {
  ...
  async function seedUnownedOrphan(
    relPath: string,
    content: string,
  ): Promise<void> {
  ...
  it("--check surfaces the unowned orphan and does not suggest --write would fix it", async () => {
    ...
    expect(res.stderr).toMatch(/review the file/i);
  });
});
...
describe("adapter placeholder dir symlink escape — CLI error mapping (security)", () => {
  ...
  async function linkDirOutside(rel: string): Promise<string> {
    const outside = await mkdtemp(
      join(tmpdir(), "code-pact-placeholder-escape-"),
    );
...
describe("adapter generated-file symlink escape — no pre-failure model pin (security)", () => {
  ...
  async function linkFileOutside(
    rel: string,
  ): Promise<{ outside: string; target: string }> {
  ...
  it("install --model with CLAUDE.md symlinked outside → refusal, profile not pinned, target unwritten", async () => {
    ...
    const parsed = JSON.parse(res.stdout) as {
      ok: true;
      data: {
        files: Array<{ relPath: string; action: string; reason?: string }>;
      };
    };
    expect(
      parsed.data.files.find(f => f.relPath === "CLAUDE.md"),
    ).toMatchObject({
      action: "refuse",
  ...
  it("upgrade --write --model with CLAUDE.md symlinked outside → refusal, profile not pinned", async () => {
    ...
    const parsed = JSON.parse(res.stdout) as {
      ok: true;
      data: {
        plan: Array<{ relPath: string; action: string; reason?: string }>;
      };
    };
...
describe("adapter DANGLING symlink escape — CLI error mapping (security)", () => {
  ...
  async function linkDanglingInternal(
    rel: string,
    missingName: string,
  ): Promise<void> {
    await rm(join(dir, rel), { recursive: true, force: true });
  ...
  it("install with `.code-pact/adapters` dangling INSIDE the project → ADAPTER_MANIFEST_INVALID, no partial state", async () => {
    ...
    const before = await readFile(profilePath, "utf8");
    await linkDanglingInternal(
      join(".code-pact", "adapters"),
      join(".code-pact", "missing-adapters"),
    );
    const res = runCli([
      "adapter",
      "install",
...
describe("adapter wrong-type write path — CLI error mapping (security)", () => {
  ...
  it("install --model with context_dir occupied by a regular file → CONFIG_ERROR, no pin", async () => {
    ...
    expect(await readFile(join(dir, profileRel), "utf8")).not.toContain(
      "model_version",
    );
  });
...
describe("adapter forged-manifest + profile → arbitrary file overwrite is REFUSED (security)", () => {
  ...
  it("install does NOT overwrite a victim file the forged manifest claims (profile contract refuses, exit 2)", async () => {
    ...
    await writeManifest(dir, "claude-code", {
      ...
      files: [
        {
          path: VICTIM,
          sha256: computeContentHash(VICTIM_CONTENT),
          managed: true,
          role: "instruction",
        },
      ],
  ...
  it("a symlinked owned skills dir cannot escape the overwrite gate (lexical-owned != real target)", async () => {
      ...
      files: [
        {
          ...
          sha256: computeContentHash(VICTIM),
          managed: true,
          role: "skill",
        },
      ],
    ...
    const entry = parsed.data.files.find(
      f => f.relPath === ".claude/skills/context.md",
    );
...
describe("adapter install — divergent managed file is surfaced, not silent (security)", () => {
  ...
  it("install --force --json → files[].action refuse + refused[] for the divergent file", async () => {
    ...
    expect(parsed.data.files.find(f => f.relPath === "CLAUDE.md")?.action).toBe(
      "refuse",
    );
  });
});
