...
For eligible single-file tasks, `code-pact task execute <id> --executor-file <project-relative-posix-path> [--agent <a>] [--timeout <ms>]` runs an external one-shot executor. The task must read and write a single existing source file, HEAD and the git index must be unchanged, and the working tree must be clean. The executor file must be given as the raw project-relative POSIX path exactly as stored in the project: no leading `/` or `~`, no `..` or `.` segments, no empty segments, and no backslashes. It must resolve to a regular, non-symlink, executable file inside the project. The executor is a trusted executable: it runs with `cwd` set to an OS temporary directory, a sanitized environment (known repository-path variables such as `PWD`, `INIT_CWD`, `npm_package_json` etc. removed), and the same process privileges as code-pact. It is not an OS sandbox.

The external process receives a JSON input (schema version 1) with the task goal, `source_path`, source content, and verification command; it must emit either a `replace_exact` payload (`expected_file_sha256` is 64 lowercase hex, `old_text` is non-empty, `new_text` is a string) or a `blocked` reason. `new_text` may be empty to delete `old_text`. The runtime validates the output at the controller boundary, applies the replacement atomically, and re-runs the verification command. The resulting source file is also bounded to `MAX_SOURCE_BYTES` (`8192` bytes); exceeding it produces `EDIT_REJECTED` with reason `RESULTING_SOURCE_TOO_LARGE`. After the edit and again after verification passes, the runtime asserts that the source file content is exactly the applied edit (regular file, bounded, valid UTF-8) and that HEAD, the index, and the working tree contain only the expected source-file change. Unborn HEAD detection is structural: `git rev-parse --verify --quiet HEAD^{commit}` is tried first, and only a `symbolic-ref`/`show-ref` pair that proves the branch ref does not yet exist is treated as an unborn repository. Scope auditing covers HEAD, the index, and Git-visible tracked/untracked paths; ignored paths and repository-external side effects are not prevented. On `EXECUTOR_MUTATED_WORKTREE` or `EXECUTION_SCOPE_VIOLATION` the runtime reports `data.rollback` (`complete`/`incomplete`/`stale`/`not_attempted`), `data.head_changed`, and `data.index_changed`; it never resets HEAD or unstages changes. Unknown executor or external mutations detected before Code Pact applies its own edit are reported with `data.rollback: "not_attempted"` and `data.rollback_reason`; they are not rolled back because their provenance cannot be proven. Known Code Pact edits are always rolled back using the captured applied content as the CAS expected value, even when git state inspection later fails; such snapshot failures emit `GIT_STATE_UNAVAILABLE` with `data.source_rollback` (`complete`/`stale`/`failed`/`not_needed`). Porcelain status characters and UTF-8 paths are validated fail-closed, and HEAD is captured before and after status acquisition; a HEAD mismatch yields `GIT_SNAPSHOT_CHANGED_DURING_READ`. Path samples are bounded to 4,096 raw UTF-8 bytes and 20 entries; public failure reasons are bounded to 2,048 UTF-8 bytes.

Public error codes for `task execute` are: `EDIT_REJECTED`, `EXECUTION_BLOCKED`, `EXECUTION_INELIGIBLE`, `EXECUTOR_FAILED`, `VERIFICATION_FAILED`, `ROLLBACK_FAILED`, `ROLLBACK_STALE_FILE`, `ROLLBACK_INCOMPLETE`, `WORKTREE_NOT_CLEAN`, `EXECUTOR_MUTATED_WORKTREE`, `EXECUTION_SCOPE_VIOLATION`, `GIT_STATE_UNAVAILABLE`, and `GIT_SNAPSHOT_CHANGED_DURING_READ` (the latter is surfaced through `git_state_unavailable` and internal git-snapshot diagnostics). Path lists are emitted as bounded summaries (`{ changed_path_count: number, changed_paths: string[], paths_truncated: boolean }`) and `VERIFICATION_FAILED` also carries a bounded `failure` projection.
