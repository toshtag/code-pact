...
> **Relationship to the canonical loop.** For starting work directly, the recommended entry point is `task prepare` (see [per-task-loop.md](../per-task-loop.md)). The runbook emits `task context` in its ready-task sequence below; that command string is kept as-is for contract stability. `task context` remains a supported diagnostic, and full-detail `task prepare` (or any explicit budget flag) returns the same pack metadata.
...
- **Bundling `recommend` output into `task runbook`** — separate commands by design; full-detail `task prepare` (or any explicit budget flag) is the surface that bundles them.
