...
The repo is already initialized (`.code-pact/` exists as the maintainer's working state). This repo **commits the per-event progress ledger** under `state/events/`, exactly as a user project does: it is shared, merge-safe operational state (the collaboration-safe-state RFC made event files conflict-free), and tracking it satisfies the committed-ledger precondition of the branch-drift gate (`CONTROL_PLANE_BRANCH_NOT_DRIVEN`), so that gate is **dogfoodable** here — e.g. `code-pact doctor --base-ref main` — instead of silently skipping on an untracked ledger. (This repo's own CI does not yet invoke the gate with `--base-ref`; satisfying the precondition is the prerequisite for that, and wiring it into CI is a separate follow-up.) Until the first real event file is committed, `.code-pact/state/events/.gitkeep` keeps the ledger path tracked so the gate does not silently skip; the ledger readers ignore any name that is not an `<at-compact>-<id>.yaml` event file, so the sentinel never affects derived state. The `.gitignore` ignores only the machine-local / derived paths under `.code-pact/`: `locks/`, `cache/`, `adapters/`, and the **legacy monolithic `state/progress.yaml`** — the maintainer's pre-event-file history, read-merged locally; per RFC A3 it stays ignored until per-event files replace it (committing the monolithic file would reintroduce the merge problem the event ledger fixes). The shared config (`project.yaml`, profiles, `state/baselines/`) is tracked. The **adapter manifest** (`.code-pact/adapters/`) is **intentionally ignored** here: this repo regenerates and ignores the adapter-owned output (`CLAUDE.md`, `.claude/skills/*`), so manifest and its generated files travel together or not at all. `design/` remains the committed source of truth for the active roadmap and not-yet-archived phase/task definitions, so `init` is not needed.

This page is the **day-to-day quick guide**. Deeper, lower-frequency maintainer topics — planning wizards, Claude model pinning, adapter-upgrade internals, Spec Kit import, release-prep posture, and the design-vs-progress contract — live in [maintainers/operations.md](maintainers/operations.md).
...
For an _activity_ (not structural) view — read-only, no lock — what is in flight,
...
```sh
code-pact task prepare <task-id> --agent claude-code --json   # entry point: minimal work order; use --detail full (or a budget flag) for recommendation + commands
...
Planning wizards, Claude model pinning, Spec Kit import, and release-prep detail → [maintainers/operations.md](maintainers/operations.md).
