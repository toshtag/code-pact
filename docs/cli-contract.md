...
- `ADR_COMMITMENTS_EMPTY` (advisory, `affects_exit: false`) — an accepted ADR referenced by a gated task that records no implementation commitments (no `## Implementation commitments` section, or one with zero checkbox items); `details.has_section` / `details.item_count`
