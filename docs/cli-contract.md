...
| `DELETE_INTENT_RECOVERY_FAILED` (v2.0, archive-level compaction — Layer 4) | 2 | A delete-intent journal (`.code-pact/state/archive/delete-intent.json`) is CORRUPT — unreadable / not the writer's canonical bytes — and cannot be auto-recovered. Surfaced by `state archive-retention --write` and `state archive-maintain --write` (which recover first), AND by `state compact-archive --write` (which is not recovery-first, so it REFUSES with this code when the pending journal is corrupt). Fail-closed: nothing is planned or deleted. | `data.recovery_pending` is `true`, `data.journal_status` is `"corrupt"`. A blind re-run fails the same way — the journal is the recovery authority, so inspect/repair (or intentionally remove it after verifying the archive state); a `present` (valid) journal, by contrast, is completed both-or-neither on the next recovery-first run |
