...
```json
{
  ...
  "data": {
    ...
    "deferred_context": {
      "manifest_ref": "context:sha256:<digest>",
      "sections": [{ "name": "completed_tasks", "bytes": 1234 }],
      "persisted": true,
      "retrieve_command": "code-pact context show context:sha256:<digest> --list --json"
    },
...
- `deferred_context` (P52) is present only when an explicit budget caused at
  ...
  single opaque reference for the pack, and `sections[]` lists only `{ name,
  bytes }` for each deferred section in actual deferral order. Normal `task
  prepare` materializes and verifies the manifest before writing the context
  pack, so `persisted: true` and `retrieve_command` is a runnable `context show
  ... --list --json` command. `task prepare --dry-run` and `task context --json`
  may compute the same `manifest_ref` and `sections`, but they are read-only:
  `persisted: false` and `retrieve_command: null`. Consumers must not infer or
  run a retrieval command when `retrieve_command` is null.
- `commands` (v1.27+, P40; updated by P51) is a complete, **mode-agnostic lookup table** — all keys are present in every `lifecycleMode`. `commands.verify` and `commands.complete` include `--json --detail agent` so an agent receives the compact failure capsule by default instead of full raw command output. The key is **exactly `record-done`** (hyphen; read it as `commands["record-done"]`, not `record_done`). It is the one entry **not runnable verbatim**: `--evidence` is agent-supplied, so it is emitted as a template with the `"<verification you ran>"` token. `next_action.message` (not `commands`) is the lifecycle-aware "what next" surface — for a `record_only` task it points at `task record-done` (a lighter loop, not lighter verification); for a `decision_loop` task it says to resolve the gating ADR first (it does **not** decide complete-vs-record-done); for `full_loop` it is the standard start→implement→verify→complete wording. Only the workable states (`start_task` / `continue_implementation`) vary by mode.
