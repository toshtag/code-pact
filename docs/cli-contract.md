...
7. **Progress record**. Records `{ task_id, status: "done", at, actor: "agent", agent, evidence: [<--evidence>], notes?, source: "external", author? }` as one new event file under `.code-pact/state/events/`, via the same lock-free per-event write as `task complete`. `author` is captured as in `task complete` (Collaboration UX RFC, D1).
8. **`--dry-run`**. Skips the record; returns `{ ok: true, data: { dry_run: true, would_append: <event> } }`. No event file is written.
