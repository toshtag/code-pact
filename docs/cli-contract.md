...
`--timeout` is a **per-command** limit in milliseconds. It defaults to `300000` (five minutes) and accepts safe integers from `1` through `2147483647`. Invalid, fractional, non-finite, zero, negative, or larger values are `CONFIG_ERROR` (exit 2). `--dry-run` previews shell commands instead of executing them.
