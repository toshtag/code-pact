...
| `PHASE_SNAPSHOT_INVALID` (v2.0, design-docs-ephemeral) | `task context` / `task prepare` / `task status` / `task start` / `task block` / `task resume` / `task complete` / `task record-done` / `task finalize` / `task runbook` (exit 2) — **and** an issue-level diagnostic in `plan lint` / `doctor` (see [Plan diagnostic codes](#plan-diagnostic-codes)) | A roadmap-referenced phase file is missing **and** its archive snapshot (`.code-pact/state/archive/phases/<id>.json`) cannot release it — corrupt / schema-invalid / identity-mismatched (`phase_id` / `original_path` / `path_sha256`) / non-terminal / task-id-colliding against the current live+archived graph. A dual-surface code: the shared task resolver (`resolveTaskInRoadmap`) and the plan-state loaders throw it as the top-level `error.code` for the task commands, while the lenient-loader surfaces (`plan lint`, `doctor`) report it as a `data.issues[]` entry. Fail-closed: a hand-deleted **completed** phase is tolerated only by a fully valid, identity-checked terminal snapshot; a present live file is never released by a snapshot (live-wins) |
...
| `MISSING_PHASE_FILE` | error | `plan lint` | `roadmap.yaml` references a phase file that does not exist (and no valid archive snapshot covers it — see `PHASE_SNAPSHOT_INVALID`) |
| `PHASE_SNAPSHOT_INVALID` (v2.0, design-docs-ephemeral) | error | `plan lint` / `doctor` (issue-level); **also a top-level `error.code`** on the `task *` commands — see [Public codes](#public-codes-top-level-error-envelopes) | `roadmap.yaml` references a phase file that does not exist **and** an archive snapshot (`.code-pact/state/archive/phases/<id>.json`) exists for it but cannot release the missing phase — it is corrupt / schema-invalid / identity-mismatched (`phase_id` or `original_path` or `path_sha256`) / non-terminal / task-id-colliding against the current live+archived graph. Fail-closed: a hand-deleted **completed** phase is tolerated only by a fully valid, identity-checked terminal snapshot; anything short is this error, never a silent pass. When the live phase file is present the snapshot is never consulted (live-wins). |
