...
`--detail` is valid only with `--json`. Omitted or `full` preserves the existing JSON shape; internal capture metadata such as stdout/stderr truncation flags is not emitted there. `--detail agent` returns a compact projection and bounds the final serialized CLI JSON line below 24 KiB. On failure, `error.message` is a short fixed string; bounded details live under `data.failure`. If the outer envelope must omit fields to stay within the bound, `data.projection_truncated: true` and `data.omitted_fields[]` identify what was omitted; executable suggestions are omitted rather than truncated. `data.failure` contains a schema-versioned failure capsule with `kind`, command metadata, deterministic `fingerprint`, UTF-8-safe stdout/stderr excerpts, `evidence_ref`, and `retrieve_command`; `data.verify` contains check summaries and successful command metadata only. Raw stdout/stderr are not duplicated in `checks[]` or `commands[]` in this mode. For command-output failures, the bounded captured stdout/stderr are stored under `.code-pact/cache/evidence/<sha256>.json`; the reference format is `evidence:sha256:<64 lowercase hex>`. Decision/state failures do not create empty evidence files. If the evidence cache write fails, the top-level error remains `VERIFICATION_FAILED`; `data.failure.evidence_available` is `false`, and `data.failure.evidence_error` carries `code: "EVIDENCE_UNAVAILABLE"`, a stable `cause_code`, and an optional diagnostic `system_code`.
