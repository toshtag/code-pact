...
`suggested_next_command` is a **rerun command to execute *after fixing* the reported `first_failure`**. It does **not** imply that rerunning without changes will resolve the failure. Human output (non-`--json`) prints the cause and a `rerun after fixing:` line to stderr below the existing generic message. No new error codes are introduced and `data.verify.checks` is unchanged, so any consumer that ignores unknown fields is unaffected.
