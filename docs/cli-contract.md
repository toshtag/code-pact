...
**`--write` success shape** (`data.mode === "written"`, exit 0): `data.keep_latest` + `data.results[]`, one per kind `{ kind, deleted[], vanished[], skipped[] }` — `deleted` the loose ids unlinked (old truth dropped), `vanished` the ids already gone at gate/unlink time (ENOENT, idempotent), `skipped` the per-record `{ id, reason }` not deleted (`needs_bundle_member_removal` / `requires_atomic_pair_removal` / `path_escape` / `unreadable` / `authority_changed` / `authority_invalid` / `unlink_failed`) — nothing is ever silently dropped.

**Why a phase snapshot ↔ event_pack pair is not deleted here.** The two are mutually bound: the pack carries the snapshot's `snapshot_sha256` (a pack *without* its snapshot is structurally broken), AND the snapshot's `progress_events` evidence resolves its `event_ids` from the durable ledger (loose events ∪ validated packs) — once the loose events are compacted into the pack, the pack is that evidence's *only* durable source, so a snapshot *without* its pack dangles (`validate` / `plan lint` / `doctor` would flag `unresolved`). A filesystem cannot unlink two files atomically, so a sequential two-unlink "both" is **not crash-safe**: a process crash, or a per-record gate skip / `unlink_failed` / `unreadable` between the two unlinks, leaves exactly one side and breaks the archive. So this layer **never** deletes a pair — a bound record is deferred whole with `skipped: requires_atomic_pair_removal` (a would_drop `event_pack` always has a would_drop phase, so it is always deferred; a `phase_snapshot` is deleted only when it has no pack). If the event_pack store is only a **partial view** (the planner emits a `(store)` block — its loose dir or bundle store was unreadable), no phase snapshot is removable (we cannot prove it has no pack): fail-closed. Decisions are independent (an archived snapshot carries no `decision_refs`) and delete last. (`authority_changed` = the loose bytes no longer match the `loose_sha256` the plan captured — swapped under us; `authority_invalid` = the loose file changed and no longer authority-validates. Both fail the planned-bytes gate and are kept.) Atomic pair removal (with a delete-intent journal + crash recovery) is a later layer.
