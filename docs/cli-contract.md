...
(v2.0, archive-level compaction — Layer 4) — `state archive-retention [--keep-latest N] [--json]` plans **keep-latest-N retention** of the archive: it bounds the archive's UNREFERENCED tail so a long project's `.code-pact/state/archive` does not grow forever. **DRY-RUN ONLY** for now — it mutates nothing; `--write` is rejected (`CONFIG_ERROR`) until the destructive retention-delete layer lands (its safety model differs from compaction's, so it is a separate verb, NOT `compact-archive --retain`). It bounds `.code-pact/state/archive` ONLY — it NEVER touches a live `design/` doc.
...
**Dry-run success shape** (`data.mode === "dry_run"`, exit 0): `data.keep_latest` (the resolved N) + `data.retention_plans[]`, one per kind `{ kind, would_keep[], would_drop[], blocked[] }`. Each item is `{ kind, id, snapshotted_at, source: "loose"|"bundle"|"both", action: "would_keep"|"would_drop"|"blocked", reason, references? }` where `reason` is one of `within_keep_latest` / `older_than_keep_latest` / `referenced_by_roadmap` / `referenced_by_live_task_dependency` / `referenced_by_decision_link` / `dependent_on_kept_phase_snapshot` / `invalid` / `ambiguous` / `reference_scan_failed`, and `references[]` (when referenced) names the live edge (`{ type, from, to }`) so "why isn't this dropped?" is answerable. Config faults (`--write`, `--keep-latest 0` / non-integer, a positional) → `CONFIG_ERROR` (exit 2).

```sh
code-pact state archive-retention --json                 # dry-run: retention_plans[] per kind (default keep-latest 20)
code-pact state archive-retention --keep-latest 5 --json # keep the latest 5 unreferenced per kind
```
