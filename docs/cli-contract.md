...
| `DECISION_PRUNE_WRITE_FAILED` | `decision prune --write` | A write could not complete **after** preflight passed: an unreadable ledger caught in preflight (zero writes); a **source edited since preflight** (`rewrite_links` — the edit is refused, never clobbered); the **record disappearing** before the delete (`delete_record` — reported honestly, not claimed as a removal); or a commit-time `rename`/`unlink` I/O error (disk full, permissions, a path that became a directory). `data` is `{ mode: "write", decision, phase, partial_applied, message }` where `phase` is `append_ledger` \| `rewrite_links` \| `delete_record` and `partial_applied` is whether any mutation already landed (ledger first + record deleted last ⇒ `append_ledger` is always `false`, the others `true`). Exit 2; inspect the working tree when `partial_applied` is `true`, then re-run |
