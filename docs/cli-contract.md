...
| `EVIDENCE_CONFLICT`                                                      | `verify --detail agent`, `task complete --detail agent`                                                                                                                                                                                                                                                                                                                                                                                            | Content-addressed evidence path already exists with bytes that do not match the canonical artifact for the same digest; fail-closed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
`--detail` is valid only with `--json`. Omitted or `full` preserves the existing JSON shape. `--detail agent` returns a compact projection: on failure, `data.failure` contains a schema-versioned failure capsule with `kind`, command metadata, deterministic `fingerprint`, UTF-8-safe stdout/stderr excerpts, `evidence_ref`, and `retrieve_command`; `data.verify` contains check summaries and successful command metadata only. Raw stdout/stderr are not duplicated in `checks[]` or `commands[]` in this mode. For command-output failures, the bounded captured stdout/stderr are stored under `.code-pact/cache/evidence/<sha256>.json`; the reference format is `evidence:sha256:<64 lowercase hex>`. Decision/state failures do not create empty evidence files.
...
With `--json --detail agent`, `task complete` preserves `error.code` and `error.cause_code`, but replaces the full failure detail under `data` with the same compact `failure` + `verify` projection documented for standalone `verify`. No progress event is recorded on this failure path; the only side effect is the derived evidence cache write for command-output failures.
