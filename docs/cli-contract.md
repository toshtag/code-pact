...
| `.code-pact/cache/context/*.json`                                                                                 | `task prepare` with an explicit context budget that defers one or more sections                                                                                                  | Derived, content-addressed cache of exact deferred context sections. Ignored by default; retrieve via `code-pact context show <ref>`. Normal `task prepare` writes and readback-verifies the manifest before atomically writing the context pack. `task context`, `task prepare --dry-run`, and `buildContextPack()` do not create this directory or write artifacts.                               |
