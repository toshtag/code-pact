...
**`--write` success shape** (`data.mode === "written"`, exit 0): `data.keep_latest` + `data.results[]`, one per kind `{ kind, deleted[], vanished[], skipped[] }` — `deleted` the loose ids unlinked (old truth dropped), `vanished` the ids already gone at gate/unlink time (ENOENT, idempotent), `skipped` the per-record `{ id, reason }` not deleted (`needs_bundle_member_removal` / `parent_phase_snapshot_kept` / `path_escape` / `unreadable` / `authority_invalid` / `unlink_failed`) — nothing is ever silently dropped. Order: phase snapshots, then event packs (each dropped only AFTER its phase snapshot was deleted this run), then decisions.
