...
| `TASK_DECISION_REF_NOT_FOUND` | error / warning | `decision_refs` path does not exist on disk. **Status-aware:** `error` while the referencing task is live; for a **`done`** task (or a `done` phase) it downgrades to `warning` (`affects_exit: false`, `details.historical: true`) — the gate already passed, so the missing ref is a historical annotation, not a live requirement. Lets a shipped decision record be retired without breaking the plan. See [decision-lifecycle-rfc](../design/decisions/decision-lifecycle-rfc.md) |
...
| `TASK_ACCEPTANCE_REF_NOT_FOUND` | error / warning | `acceptance_refs` path does not exist on disk. **Status-aware** like `TASK_DECISION_REF_NOT_FOUND`: `error` while the task is live, `warning` (`affects_exit: false`, `details.historical: true`) for a `done` task / phase |
