...
`writes` declarations are not existence-checked. `acceptance_refs` must name a
present project-contained path; URL strings, commands, missing paths, unsafe
paths, project-outside symlinks, manual tests, screenshots, logs, comments, PR
descriptions, verification commands, and Failure Capsule output are not static
regression evidence. The advisory is declaration-only: it does not read test
content and does not prove that a test prevents a runtime failure.
