...
| `CONTROL_PLANE_GITIGNORED` (v1.32+) | warning | **Part of the shared control plane is git-ignored.** A `.gitignore` rule matches one or more shared areas — `project.yaml`, `agent-profiles/`, `model-profiles/`, `state/baselines/`, `state/events/` (the `message` names which) — so that state never reaches git; collaboration state stays local and `CONTROL_PLANE_BRANCH_NOT_DRIVEN` silently skips. Usual cause: a blanket `/.code-pact/` ignore (or a file-scoped `state/events/*.yaml`) that overrides the narrow entries `init` writes (`init` never deletes a user's existing lines). Authoritative via `git check-ignore --no-index` over a representative **file** in each area (rule-only, so a force-added file does not mask it; negation re-includes are honoured). Silent skip when git is unavailable / not a repo or `.code-pact/project.yaml` is absent. Advisory — `doctor` / default `validate` do not fail on it; `validate --strict` promotes it (like other doctor warnings). Silence via `disabled_checks: [CONTROL_PLANE_GITIGNORED]` |
...
> **`conflicts[]` shipped in D3 (v1.33+).** D2 (v1.32) shipped the four activity buckets with no `conflicts` key; D3 added `data.conflicts[]` once attribution (D1) could populate the "who". A D2-era consumer that ignores unknown keys is unaffected (the array is additive). The `details.events[]` shape is shared with the `plan analyze` / `doctor` `PROGRESS_EVENT_CONFLICT` surfaces — see § Plan diagnostic codes.
