...
| `ARCHIVE_BUNDLE_WRITE_FAILED` (v2.0, archive-level compaction — Layer 2/3) | 2 | `state compact-archive --write` could not write an archive bundle or verify its readback (a build/write/verify fault: a non-canonical or Tier-1-invalid member, an atomic-write failure, or a readback divergence) | Read `error.message` for the phase (`build` / `write_bundle` / `verify_bundle`); nothing past the fault point was mutated. Re-run after fixing the offending record, or remove a stale bundle at the named path |
...
(v2.0, archive-level compaction — Layer 4) — `state compact-archive [<kind>] [--write] [--json]` folds the loose per-item archive records of a kind (`phase_snapshot` → `archive/phases/*.json`, `event_pack` → `archive/event-packs/*.json`, `decision_record` → `archive/decisions/*.json`) into content-addressed **bundles** (`archive/bundles/<kind>-<hash>.json`), then **deletes** the loose copies a verified bundle now holds byte-identically. A bundle is the durable form; readers resolve `loose ∪ bundle` (loose wins), so deleting a bundled loose record strands nothing. **Dry-run by default** (writes/deletes nothing); pass `--write` to apply under the [advisory write lock](#public-codes-top-level-error-envelopes) (`LOCK_HELD` on contention). The optional positional `<kind>` restricts the run to one kind; omitting it processes all three. NOTE the ids reported are **logical record ids** — a `deleted` id no longer has a loose file (it lives only in a bundle). This verb does **not** yet bound the bundle count / supersede a stale member / shard (retention + supersession are later Layer-4 work).

**Dry-run success shape** (`data.mode === "dry_run"`, no mutation, exit 0): `data.plans[]`, one entry per kind, each `{ kind, would_bundle[], would_delete[], would_skip[] }` — `would_bundle` = loose ids not yet in any bundle (would be folded into a new bundle); `would_delete` = loose ids a verified bundle already holds byte-identically (would be removed); `would_skip` = loose ids that cannot be acted on fail-closed (`{ id, reason }` where reason is `bundle_stale` — a same-id bundle member with different bytes — or `bundle_member_invalid`).
