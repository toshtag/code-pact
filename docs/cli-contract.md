...
| `STATE_COMPACT_INELIGIBLE` (v2.0, event-pack compaction Layer 2) | `state compact`, `state compact --write` | `state compact <phase-id>` cannot compact the phase. `data.block.kind` is one of: `phase_file_still_present` (the live `design/phases/<id>.yaml` still exists — run `phase archive <id> --write` first), `snapshot_missing` / `snapshot_invalid` (no/corrupt phase snapshot), `snapshot_evidence_broken` (the snapshot's `progress_events` evidence does not resolve from the durable ledger — loose ∪ packs), `pack_stale` (an existing pack's event set differs from the current loose files), `pack_invalid` (an existing pack failed Tier-1/binding), or `candidate_bind_failed` (an internal consistency guard). Identical in dry-run and `--write`. Exit 2 |
| `STATE_COMPACT_WRITE_FAILED` (v2.0, event-pack compaction Layer 2) | `state compact --write` | The pack write or readback-verify failed. `data.phase` is `write_pack` (`partial_applied:false` — the pack is NOT on disk; e.g. a concurrent writer created it) or `verify_pack` (`partial_applied:true` — the pack IS on disk but failed readback; `data.next_action` says to inspect/remove it and rerun — Layer 2 does not unlink, so it does not auto-recover). Exit 2 |
