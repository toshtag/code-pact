...
**`--write` success shape** (`data.mode === "written"`, exit 0): `data.keep_latest` + `data.results[]`, one per kind `{ kind, deleted[], vanished[], skipped[] }` — `deleted` the loose ids unlinked (old truth dropped), `vanished` the ids already gone at gate/unlink time (ENOENT, idempotent), `skipped` the per-record `{ id, reason }` not deleted (`needs_bundle_member_removal` / `dependent_event_pack_not_removable` / `path_escape` / `unreadable` / `authority_changed` / `authority_invalid` / `unlink_failed`) — nothing is ever silently dropped. **Order: event packs FIRST, then phase snapshots, then decisions** — because an `event_pack` BINDS to its phase snapshot (the pack carries the snapshot's `snapshot_sha256`; a pack without its snapshot is structurally broken, while a snapshot without its pack is fine). So a phase snapshot whose dependent `event_pack` could **not** be removed this run (the pack is bundle-only / `both`, i.e. `needs_bundle_member_removal`, or its unlink failed) is itself held back with `skipped: dependent_event_pack_not_removable` — deleting the snapshot first would orphan the surviving pack and break readers. (`authority_changed` = the loose bytes no longer match the `loose_sha256` the plan captured — swapped under us; `authority_invalid` = the loose file changed and no longer authority-validates. Both fail the planned-bytes gate and are kept.)
