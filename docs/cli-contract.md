...
**`--write` success shape** (`data.mode === "written"`, exit 0): `data.keep_latest` + `data.results[]`, one per kind `{ kind, deleted[], vanished[], skipped[] }` — `deleted` the loose ids unlinked (old truth dropped), `vanished` the ids already gone at gate/unlink time (ENOENT, idempotent), `skipped` the per-record `{ id, reason }` not deleted (`needs_bundle_member_removal` / `dependent_event_pack_not_removable` / `parent_phase_snapshot_not_removable` / `path_escape` / `unreadable` / `authority_changed` / `authority_invalid` / `unlink_failed`) — nothing is ever silently dropped.

**A phase snapshot and its `event_pack` are removed BOTH-OR-NEITHER**, because they are mutually bound: the pack carries the snapshot's `snapshot_sha256` (a pack *without* its snapshot is structurally broken), AND the snapshot's `progress_events` evidence resolves its `event_ids` from the durable ledger (loose events ∪ validated packs) — once the loose events are compacted into the pack, the pack is that evidence's *only* durable source, so a snapshot *without* its pack dangles. So a pack/snapshot pair is deleted only when **both** copies are loose-only `would_drop` removable this run; if either side is bundle-only / `both` / blocked / unlink-failed, **both are kept** — the pack `skipped: parent_phase_snapshot_not_removable` and the snapshot `skipped: dependent_event_pack_not_removable`. If the event_pack store is only a **partial view** (the planner emits a `(store)` block — its loose dir or bundle store was unreadable), no phase snapshot is removable (a pack we could not enumerate might be bound to it): fail-closed. Within a removable pair the pack is unlinked **first**, then the snapshot only once the pack is gone this run, so a crash mid-pair never leaves a snapshot bound to a deleted pack. Decisions are independent (an archived snapshot carries no `decision_refs`) and delete last. (`authority_changed` = the loose bytes no longer match the `loose_sha256` the plan captured — swapped under us; `authority_invalid` = the loose file changed and no longer authority-validates. Both fail the planned-bytes gate and are kept.)
