...
**Root cause on the error face (v1.27+, P39 — additive).** `task complete` also sets `error.cause_code` so an agent reading only `error` knows what failed without dropping into `data`, and `error.message` becomes actionable. The message is chosen by the **first failing check's name** (not a verbatim copy of `first_failure.reason`); the command cause additionally embeds that reason:
