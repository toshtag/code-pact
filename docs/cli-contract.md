...
- `TASK_CONTEXT_BUDGET_UNACHIEVABLE` (v1.30+, P50, advisory, `affects_exit: false`) — the recommended context budget cannot fit even after maximal eligible elision (`minimum_achievable_bytes > budget_bytes`, the same shared floor `CONTEXT_OVER_BUDGET` uses); `details.profile` / `details.budget_bytes` / `details.minimum_achievable_bytes`
