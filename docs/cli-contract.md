...
| `ARCHIVE_BUNDLE_INVALID` (v2.0, archive-level compaction — Layer 1a, reader only; not yet emitted by a CLI surface) | error | strict archive readers (lenient surfaces will drop the bundle in a later layer) | An archive bundle (`.code-pact/state/archive/bundles/<kind>-<hash>.json`, which folds many per-item archive records of one kind for [bounded-archive compaction](../design/decisions/archive-level-compaction-rfc.md)) failed **Tier-1** self/bijection validation: schema, per-member `sha256`↔canonical-bytes match, in-bundle duplicate id, ascending-id order, or the `member_ids_sha256` set checksum. Same fail-closed family as `EVENT_PACK_INVALID`; Tier-2 per-member binding + loose∪bundle wiring are later layers. |
