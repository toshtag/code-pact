...
When a retained prior failure has the same exact command-output fingerprint, `task complete --json --detail agent` may add `data.prior_local_signal` with `schema_version`, `exact_match_count`, and `last_observed_at`; the object is advisory, canonical JSON <= 1 KiB, omitted on first observation, and never added to default/full JSON. When `data.prior_local_signal` is present, agents must not rerun the same hypothesis or unchanged fix; change the repair hypothesis, or stop and report that additional information is missing.
