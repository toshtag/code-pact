...
| `EVENT_FILE_ID_MISMATCH` (collaboration-safe-state RFC, B1/B5) | per-event progress ledger reads/writes | Data-integrity invariant: an event file's content (or its stored `id`) does not match its filename, which **is** the full content id. Fail-closed so a corrupt / partial / hand-edited ledger entry is never read or written-over silently. Surfaced as an **error issue** by `doctor` / `plan analyze` (and `plan lint`'s lenient loader); propagated as a command error by the task / `verify` readers (`pack` is best-effort and skips it). |
