...
| `DECISION_PRUNE_PLAN_STALE` | `decision prune --write` | The working tree changed under the plan: re-collecting inbound links no longer reproduces the plan exactly, a span no longer byte-matches its collected `raw_link`, or the **target record** itself vanished / became a non-regular file at apply time (a concurrent edit, or a tree that moved since the plan was built). `--write` re-validates before touching disk, so this aborts with **zero writes**. `data` is `{ mode: "write", decision, stale[] }` where each `stale[]` entry is `{source_file, line, column, expected, found}`. Exit 2; re-run `decision prune` to rebuild the plan |
| `DECISION_PRUNE_WRITE_FAILED` | `decision prune --write` | A disk write failed **after** preflight passed — an unreadable ledger caught in preflight (zero writes), or a commit-time `rename`/`unlink` I/O error (disk full, permissions, a path that became a directory). `data` is `{ mode: "write", decision, phase, partial_applied, message }` where `phase` is `append_ledger` \| `rewrite_links` \| `delete_record` and `partial_applied` is whether any mutation already landed (the ledger is written before docs and the record is deleted last, so a ledger failure is always `partial_applied: false`). Exit 2; inspect the working tree when `partial_applied` is `true`, then re-run |
...
| `TASK_DECISION_REF_NOT_FOUND` | error / warning | `decision_refs` path does not exist on disk. **Status-aware:** `error` for any **not-`done`** task; for a **`done`** task it downgrades to `warning` (`affects_exit: false`, `details.historical: true`) — the task has completed, so the missing ref is a historical annotation, not a live file requirement. Keyed on the **task's own status** (a `done` phase does not loosen an open task's gate; `cancelled` stays `error`). A missing target recorded in [`design/decisions/PRUNED.md`](../design/decisions/PRUNED.md) (an intentional retirement; ledger entries are confined to top-level `design/decisions/*.md`) is **silent**; one not recorded still warns (possible accidental deletion). `acceptance_refs` are **not** silenced by the ledger. Lets a shipped decision record be retired without breaking the plan. See [decision-lifecycle-rfc](../design/decisions/decision-lifecycle-rfc.md) |
...
| `TASK_ACCEPTANCE_REF_NOT_FOUND` | error / warning | `acceptance_refs` path does not exist on disk. **Status-aware** like `TASK_DECISION_REF_NOT_FOUND`: `error` for any not-`done` task, `warning` (`affects_exit: false`, `details.historical: true`) for a `done` task (keyed on the task's own status) |
...
1. **Preflight (no writes).** The **target itself** must still be a readable, regular file — a record that vanished / became a directory / escapes the root is plan drift, not a success to absorb. The plan must still describe the live tree exactly — `--write` re-collects inbound links and refuses if anything moved (a shifted span, a link reclassified into code/image, a removed link, a **new** inbound link the plan would not rewrite, or a source that became unreadable / reference-style), then re-reads each source and confirms every span still byte-equals its `raw_link`. The ledger's next content is read+computed. A plan/tree divergence aborts with [`DECISION_PRUNE_PLAN_STALE`](#public-codes-top-level-error-envelopes); an unreadable ledger aborts with [`DECISION_PRUNE_WRITE_FAILED`](#public-codes-top-level-error-envelopes). Either way, **zero writes** (exit 2).
2. **Append the `PRUNED.md` ledger row FIRST** (created with its header when absent; existing ledgers are only appended to). A row for a still-present record is benign — the status-aware ref check only consults the ledger once the file is absent — so a ledger failure here leaves inbound docs **byte-identical**. The decision path is recorded as a **code span**, never a link.
3. **Rewrite inbound links** (per file, back-to-front, each via the shared [`atomicWriteText`](#state-file-write-guarantees) temp+rename primitive).
4. **Delete the record** — the only irreversible step, done last, after the durable tombstone row exists and links point nowhere.

Cross-file atomicity is not claimed (a POSIX filesystem cannot transact across files without a journal); the guarantee is that a failure never leaves a **broken-link or `validate`-breaking** intermediate state. A commit-time I/O failure (rename/unlink) raises [`DECISION_PRUNE_WRITE_FAILED`](#public-codes-top-level-error-envelopes) with `data.phase` (`append_ledger` / `rewrite_links` / `delete_record`) and `data.partial_applied`.
...
| `design/decisions/PRUNED.md` | `decision prune --write` | Append-only tombstone ledger: one row appended per prune (file created with a header on the first prune). Decision path recorded as a code span, never a link |
