...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [`FOCUSED_VERIFICATION_NOT_CONFIGURED`](#focused_verification_not_configured-from-verify---stage-focused)                                                                                                                    | Focused stage verification was requested but no focused policy is configured                          | Add `.code-pact/project.yaml` `verification_policy.focused_command`, or use the default/full verification path                                                                                                                              |
| [`FULL_RETRY_REQUIRES_FOCUSED_PASS`](#full_retry_requires_focused_pass-from-full-verification-or-task-complete)                                                                                                              | A full verification retry is blocked until the fix has a newer focused pass                           | Run `code-pact verify --phase <phase> --task <task> --stage focused --json --detail agent`, then retry `task complete`                                                                                                                     |
| [`FULL_VERIFICATION_BUDGET_EXCEEDED`](#full_verification_budget_exceeded-from-full-verification-or-task-complete)                                                                                                            | The configured full verification attempt budget is exhausted                                          | Stop full retries for this task; use focused failures to fix, split the task, or explicitly change the policy                                                                                                                              |
| [`VERIFICATION_LEDGER_INVALID`](#verification_ledger_invalid-from-staged-verification)                                                                                                                                        | The local verification-run ledger is corrupt or unsafe                                                | Treat `.code-pact/cache/verification-runs/ledger.jsonl` as generated cache; move/delete it locally, then rerun focused verification                                                                                                         |
| [`VERIFICATION_STATE_UNAVAILABLE`](#verification_state_unavailable-from-staged-verification)                                                                                                                                  | Code Pact could not prove the exact Git state for focused/full authorization                          | Fix the Git command failure or unsupported untracked file type, then rerun focused verification                                                                                                                                             |
...
## `FOCUSED_VERIFICATION_NOT_CONFIGURED` from `verify --stage focused`

The project requested focused stage verification, but `.code-pact/project.yaml` does not define `verification_policy.focused_command`. Add the focused command to project policy if the repository supports staged verification, or run the ordinary `verify` command without `--stage focused`.

## `FULL_RETRY_REQUIRES_FOCUSED_PASS` from full verification or `task complete`

The policy has already recorded a full verification attempt for this task/workspace digest. After making a fix, run focused verification first:

```sh
code-pact verify --phase <phase> --task <task> --stage focused --json --detail agent
```

Only retry `task complete` after that focused pass is recorded.

## `FULL_VERIFICATION_BUDGET_EXCEEDED` from full verification or `task complete`

The task exhausted `verification_policy.max_full_attempts` (default: 2). Do not keep repeating full gates. Fix from focused evidence, split the task into a new lifecycle, or intentionally change the policy with review.

## `VERIFICATION_LEDGER_INVALID` from staged verification

The staged-verification ledger is generated local cache at `.code-pact/cache/verification-runs/ledger.jsonl`. If it is corrupt, malformed, or unsafe, treat it as untrusted cache state: move or delete it locally, rerun focused verification, then proceed to full verification.

## `VERIFICATION_STATE_UNAVAILABLE` from staged verification

Code Pact could not prove the current verification state from Git. This is fail-closed: focused results are not recorded, and full verification is not authorized from an unproven state. Inspect the envelope fields such as `operation`, `exit_code`, `timed_out`, and `aborted`; fix the Git failure or unsupported untracked file type, then rerun focused verification.
