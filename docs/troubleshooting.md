...
source file is rolled back on `verification_failed`; for `execution_scope_violation`,
`executor_mutated_worktree`, and `rollback_incomplete` the source file is also restored
whenever possible.
...
| `EXECUTOR_FAILED`           | The external executor exited non-zero, timed out, or emitted malformed/oversized JSON; `data.reason` is bounded                                     | Check the executor path, permissions, timeout, and stdout size (< 24576 bytes)                                                      |
...
| `ROLLBACK_INCOMPLETE`       | The source file was rolled back, but other unexpected working-tree changes remain                                                                   | Review `data.paths` (a bounded summary) and `data.failure`, clean the working tree, and retry                                       |
| `WORKTREE_NOT_CLEAN`        | The working tree had staged, unstaged, or untracked changes before execution                                                                        | Commit or stash changes and retry                                                                                                   |
| `EXECUTOR_MUTATED_WORKTREE` | The executor modified the working tree (the source file or any other file) before returning                                                         | Fix the executor to only emit a `replace_exact` response and not touch the filesystem                                               |
| `EXECUTION_SCOPE_VIOLATION` | Verification or a side-effect altered files outside the target source file                                                                          | Ensure verification commands are read-only and do not create artifacts; review `data.paths` (a bounded summary) and `data.rollback` |
