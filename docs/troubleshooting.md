...
| `EXECUTOR_FAILED`           | The external executor exited non-zero, timed out, or emitted malformed/oversized JSON                                                               | Check the executor path, permissions, timeout, and stdout size (< 24576 bytes)                                                          |
...
| `EXECUTOR_MUTATED_WORKTREE` | The executor wrote to files outside the target source file before returning                                                                         | Fix the executor to only emit a `replace_exact` response and not touch the filesystem                                                   |
