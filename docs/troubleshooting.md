...
`execution_scope_violation`, `executor_mutated_worktree`, and `rollback_incomplete`;
rollback is best-effort and not guaranteed. The runtime never resets HEAD or
unstages index changes.
...
| `EXECUTOR_FAILED`           | The external executor exited non-zero, timed out, emitted malformed/oversized JSON, or an unsupported `kind`; `data.reason` is bounded              | Check the executor path, permissions, timeout, and stdout size (< 24576 bytes); verify `expected_file_sha256` is 64 lowercase hex                           |
...
| `EXECUTOR_MUTATED_WORKTREE` | The executor modified repository state (HEAD, index, worktree, or any file) before returning                                                        | Fix the executor to only emit a `replace_exact`/`blocked` response and not touch the filesystem or git state                                                |
