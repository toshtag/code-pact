...
- **`.code-pact/` をコミットしない**（デフォルト）。 `.code-pact/` が `.gitignore` にないと `code-pact doctor` は `LOCAL_NOT_GITIGNORED`（警告）を出します。想定される慣習は、`.code-pact/state/` 配下の運用状態は各開発者ローカルとし、コミットするのは `design/` の中身と adapter 生成の指示ファイルだけ、というものです。状態ファイルの書き込み契約の全体は [cli-contract.md](../../cli-contract.md)（英語）を参照してください。
  - **例外 — CI の branch-drift ゲート（v1.26+）。** CI で `CONTROL_PLANE_BRANCH_NOT_DRIVEN`（`validate --strict --base-ref`、P34）を使う場合、ゲートは*コミット済み*の台帳を読むため、`.code-pact/state/progress.yaml` を **git 追跡対象にする必要があります**。`.code-pact/` はデフォルト慣習で gitignore されるので、この1ファイルだけ force-add してください: `git add -f .code-pact/state/progress.yaml`。`.code-pact/doctor.yaml` の `control_plane_branch_not_driven.exclude_globs` も使うなら、その設定も CI で参照可能にする必要があります（force-add するか、追跡対象のプロジェクト設定の慣習を採用する）。これはデフォルトの「`.code-pact/` をコミットしない」を*反転*するのではなく、追跡する1ファイルに*限定する*例外です。
