...
| こんなとき | 使う経路 |
| --- | --- |
| 機能が何かを正確に把握していて、30分ほど取れる | 経路2 — 手動 |
| brief をもとに AI エージェントにフェーズ+タスク分解を起案させたい | 経路3 — AI 支援 |
| このリポジトリでインストールの動作確認をしたいだけ | 経路1 — チュートリアル（`code-pact tutorial` はこのリポジトリに何も書き込みません） |

各経路の詳細は [はじめに](../getting-started.md) を参照してください。brownfield では**手動の経路が適切なことが多い**です。あなたは既にコードベースを知っており、AI エージェントが起案できるものは5分で自分でも書けるものばかりですし、brownfield プロジェクトの `plan brief` / `plan constitution` の内容は既存ドキュメントと重複しがちです。
...
- **`.code-pact/` をコミットしない。** `.code-pact/` が `.gitignore` にないと `code-pact doctor` は `LOCAL_NOT_GITIGNORED`（警告）を出します。想定される慣習は、`.code-pact/state/` 配下の運用状態は各開発者ローカルとし、コミットするのは `design/` の中身と adapter 生成の指示ファイルだけ、というものです。状態ファイルの書き込み契約の全体は [cli-contract.md](../../cli-contract.md)（英語）を参照してください。
...
- [migration.md](../../migration.md)（英語） — 新規導入ではなく既存の `code-pact` プロジェクトをアップグレードする場合は、移行ガイドが正しい入口です。
