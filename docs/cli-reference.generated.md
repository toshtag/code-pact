...
| Flag | Value | Description |
| --- | --- | --- |
| `--executor-file` (required) | `<path>` | Project-relative POSIX path to the trusted external one-shot executor executable. Must be given exactly as stored in the project (no leading './', no '..' or '.' segments, no empty segments, no '\\'), and must be executable and accept JSON on stdin. |
| `--agent` | `<name>` | Agent name. Defaults to project default_agent. |
...
| Flag | Value | Description |
| --- | --- | --- |
| `--output` | `<path>` | Path for the review ZIP bundle (default: .code-pact/cache/reviews/<task-id>/bundle.zip). |
| `--json` | — | Emit JSON. |
