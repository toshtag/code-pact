...
- [`scripts/changelog-archive.mjs`](../../scripts/changelog-archive.mjs) (`pnpm check:changelog-archive`) fails when `CHANGELOG.md` still holds a major older than the current one (read from `package.json`); the fix is to roll it out with `pnpm changelog:archive` (moves the older major verbatim into [`history/`](history/), leaving a pointer). See [`releasing.md`](releasing.md) step 3.
