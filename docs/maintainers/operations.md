...
## Claude model pinning (v0.5.0)

`--model` pins and validates the `claude-code` profile's `model_version`. The pin drives
`--model` validation, profile seeding, the `model_map` tier defaults, and the doctor checks
below; schema-v2 `CLAUDE.md` generation is model-neutral, so the pin does not change the
generated bytes:

```sh
# Pin the claude-code profile's model_version to Opus 5:
code-pact adapter install claude-code --model opus-5

# Re-run skills only (skill files regenerated, CLAUDE.md left untouched):
code-pact adapter install claude-code --regen-skills
```

Supported values: `opus-5`, `sonnet-5`, `opus-4.8`, `opus-4.7`, `opus-4.6`, `sonnet-4.6` (and
their vendor-id aliases, e.g. `claude-opus-5`). These live in `CLAUDE_MODEL_VERSIONS` in
...
**Provider scope (Claude-only model governance).** The `--model` validator, the default
`model_map` seed, and the `MODEL_ID_UNKNOWN` / `MODEL_MAP_STALE` doctor checks all read the
bundled Claude catalog and apply to the `claude-code` profile only. None of it reaches the
generated `CLAUDE.md`, which is a model-neutral bootstrap. Other
agents' `model_map` values — e.g. `codex` (`gpt-5.5` / `gpt-5.4` / `gpt-5.4-mini`) — are
**advisory display only**: they render in those agents' generated instruction files (which
are still adapter schema v1) but are
