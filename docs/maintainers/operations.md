...
- **The exit code enforces "bounded", so you don't have to eyeball it** — `state archive-maintain --write` exits **0** only when the archive is v2.0-bounded (`bounded_status.file_count_bounded && unreferenced_old_truth_bounded`) AND the preview checks pass; it exits **1** when the archive is not yet bounded (rerun to converge — ≤ 2 runs) or a preview check failed; and **2** on a maintenance-step fault (the same error codes the low-level verbs surface). So a release script can gate on `archive-maintain --write` returning 0. Still inspect `summary.skipped` / `summary.mixed_source_deferred` (should be `0`) and the `bounded_status` reason counts to understand WHY a run is not yet bounded.
