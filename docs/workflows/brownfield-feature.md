...
You can always switch the verify command later by editing `design/phases/<phase>.yaml` directly — the active phase YAML is the editable source of truth for that verification command.
...
- **Most of `.code-pact/` is shared state — commit it; only a small local subset is ignored.** `init` ignores the machine-local / derived paths: `/.code-pact/locks/` and `/.code-pact/cache/`, plus `/.local/` (private planning notes — *this* is what the `LOCAL_NOT_GITIGNORED` warning checks) and `/.context/` (regenerable context packs). **Commit the rest**: `.code-pact/project.yaml`, `agent-profiles/`, `model-profiles/`, `state/baselines/`, and the progress ledger — `state/events/**` (one file per event; where the task verbs now write), the event packs under `state/archive/event-packs/**` (compacted per-event history for archived phases), plus the legacy `state/progress.yaml` while it contains legacy events or as the empty tracking sentinel `init` creates (task commands no longer write it). The adapter manifest (`.code-pact/adapters/*.manifest.yaml`) is committed **only together with** the adapter files it lists (e.g. `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `.claude/skills/*`, `.cursor/**`); if you ignore that generated output (as code-pact's own repo does), ignore the manifest too — a committed manifest whose files are absent fails `adapter doctor`. `design/` remains the committed source of truth for the active roadmap and not-yet-archived phase/task definitions; a completed phase's runtime truth lives in a `.code-pact/state` archive snapshot and a retired / settled decision's in a `.code-pact/state` decision record, so each historical design doc may be removed once it is snapshot/record-backed. See [the design-doc lifecycle](../concepts/design-doc-lifecycle.md) for how to archive/retire safely (and why a bare `rm` first fails closed), and [`docs/cli-contract.md`](../cli-contract.md) for the full state-file write contract.
  - **CI note.** `validate` / `recommend` / `task prepare` read the project config above, so it must be in the CI checkout. If you also use the `CONTROL_PLANE_BRANCH_NOT_DRIVEN` gate (`validate --strict --base-ref`), the **committed** ledger — `.code-pact/state/events/**`, the event packs under `.code-pact/state/archive/event-packs/**`, plus the legacy `.code-pact/state/progress.yaml` — is what it reads; commit it (it isn't auto-ignored), or `git add -f .code-pact/state/events/ .code-pact/state/archive/event-packs/ .code-pact/state/progress.yaml` if your repo deliberately ignores `.code-pact/`. Force-adding only the ledger is not enough if the config is ignored too.
