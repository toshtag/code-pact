...
- **Don't commit `.code-pact/`.** `code-pact doctor` raises `LOCAL_NOT_GITIGNORED` (warning) when `.code-pact/` is not in `.gitignore`. The expected convention is that operational state under `.code-pact/state/` is local-to-each-developer; commit only the contents of `design/` and the adapter-generated instruction files. See [`docs/cli-contract.md`](../cli-contract.md) for the full state-file write contract.
