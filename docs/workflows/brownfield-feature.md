...
- **Don't commit `.code-pact/`** *(default).* `code-pact doctor` raises `LOCAL_NOT_GITIGNORED` (warning) when `.code-pact/` is not in `.gitignore`. The expected convention is that operational state under `.code-pact/state/` is local-to-each-developer; commit only the contents of `design/` and the adapter-generated instruction files. See [`docs/cli-contract.md`](../cli-contract.md) for the full state-file write contract.
  - **Exception — the CI branch-drift gate (v1.26+).** If you opt into `CONTROL_PLANE_BRANCH_NOT_DRIVEN` in CI (`validate --strict --base-ref`, P34), the gate reads the *committed* ledger, so `.code-pact/state/progress.yaml` **must be tracked**. Because `.code-pact/` is gitignored by the default convention, force-add the one file: `git add -f .code-pact/state/progress.yaml`. If you also use `control_plane_branch_not_driven.exclude_globs` in `.code-pact/doctor.yaml`, that config must likewise be available in CI (force-add it, or adopt a tracked project-config convention). This is the one place the default "don't commit `.code-pact/`" is deliberately narrowed — to a single tracked file — not reversed.
