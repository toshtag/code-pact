...
- **`check:fs-authority`** (`scripts/check-fs-authority.mjs`): an **AST-based** gate over `src/commands/**`, `src/core/**`, and `src/cli/**`. It verifies fs operation path arguments are sourced from approved imported authority helpers or a structured allowlist entry, tracks local variable provenance, and merges branch states conservatively so a variable is authorized only when every reachable branch assigns it from an approved helper. Generic symlink-free containment is not inferred as filesystem authority. It is a structural gate, not a whole-project proof.
