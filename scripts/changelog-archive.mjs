...
// definitions trail their version section, so moving a whole section keeps each
...
export function renderChangelog(preamble, keptBlocks, archivedMajors) {
  ...
  // Drop any pre-existing pointer block among the kept blocks (idempotent re-render).
  const kept = keptBlocks.filter((b) => b.heading.trim() !== POINTER_HEADING);
  for (const b of kept) parts.push(b.text.trimEnd());
  if (archivedMajors.length > 0) parts.push(renderPointer(archivedMajors).trimEnd());
  return `${parts.join("\n\n")}\n`;
}
...
function main(argv) {
  ...
  // write. Each major is archived exactly once: once its sections leave
  // CHANGELOG.md, a re-run finds nothing to move (changed:false short-circuits
  // above), so a plain deterministic write per archive file is correct — no
  // append/dedup bookkeeping needed.
