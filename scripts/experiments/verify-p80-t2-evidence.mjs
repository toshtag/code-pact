...
try {
  ...
  if (
    manifest.pair_status !== "non_discriminating" &&
    manifest.pair_status !== "qualified" &&
    manifest.pair_status !== "not_started"
  ) {
    fail(`unexpected pair_status: ${manifest.pair_status}`);
  }
  if (manifest.capability_gate?.passed !== true) {
    fail("capability gate did not pass");
  }

  const expected = JSON.parse(
    fs.readFileSync(path.join(tmpDir, "hashes.json"), "utf8"),
  );
