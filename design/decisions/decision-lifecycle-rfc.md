...
- PR-C — `decision prune`, split (destructive work ships as small, separately-reviewable layers):
  ...
  - [x] **PR-C2** — `--write`: executes the C1c plan under the advisory write lock. Re-collects inbound links and requires the plan to still describe the live tree exactly + a per-span byte re-check (fail-closed `DECISION_PRUNE_PLAN_STALE`, **zero writes** on any divergence — a reclassified/new/removed link or shifted span). Then, in crash-safe order: rewrites inbound links (back-to-front, atomic temp+rename per file) → appends the `PRUNED.md` tombstone row (`serializePrunedRow` / `appendPrunedLedger`, path as a **code span**) → deletes the record last (the only irreversible step). Executor: `src/core/decisions/prune-executor.ts`.
