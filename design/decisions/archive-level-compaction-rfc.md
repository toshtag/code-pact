...
- A bundle (or member) failing either tier is handled by the reader's **posture**, which is ORTHOGONAL to its resolution *mode* (Resolution, below): the mode decides *when* a bundle is loaded; the posture decides *what happens when a loaded bundle fails validation*. Pick the posture by the deciding axis **referenced-item resolution (→ fail-closed) vs unreferenced / advisory discovery (→ fail-soft)** — the same split as [[step4-archive-reader-invariants]] (4a strict / 4b fail-soft) and the event-pack strict-loader-vs-lenient-surface convention:
  - **fail-closed strict** — a reader RESOLVING a referenced item (a live or archived authority points at it) treats a corrupt / failing bundle as a hard fault (`fail_invalid` / strict error; **throws** in strict loaders), because the failure may be masking the very truth requested. Readers in this posture: `resolveMissingPhaseRef`, `validate` (and `--strict`), the event-pack binder for a referenced pack, decision resolution for a required record, and every `strict-reconcile` caller.
  - **fail-soft lenient** — a reader ENUMERATING / DISCOVERING optional items (whose failure supplies no ids and introduces no graph ambiguity) **drops** the failing bundle and reports an advisory (`affects_exit:false`), never throwing. Readers in this posture: unreferenced-snapshot discovery (4b), `plan lint` advisories.
