...
Entries are confined to **`design/decisions/**.md`** decision records — a row
pointing anywhere else (a `docs/` page, a `design/phases/*.yaml`, a `../`
traversal, or `README.md` / `PRUNED.md` itself) is ignored, so the ledger can
never silence an arbitrary missing file. It is a *decision* tombstone only:
