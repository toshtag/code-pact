...
**Status:** accepted — dogfood migration plan. This RFC fixes the migration *decision* and the durable-truth model; execution started with the P22 cancelled-phase smoke archive and proceeds in follow-up PRs using **already-shipped** commands (`phase archive --write [--attest]`, `decision retire --write`, `state compact --write`). No new mechanism is required; one optional convenience (bulk-attest UX) is a non-blocking nice-to-have. (2026-06)
