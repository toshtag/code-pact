...
| — | [Control-plane v2](control-plane-v2-rfc.md) | **Accepted (scope-limited: PR0 + PR1 only).** Takes up deferred Bucket C (phase identity / glob discovery / `roadmap.yaml`-advisory / per-task files); PR2+ remain gated on the §5 soak + forensic incident backfill. |
