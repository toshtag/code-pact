...
| v2.0 | [Design docs are ephemeral](design-docs-ephemeral-directive.md) → *transitional directive* | **Proposed (v2.0 product direction).** Relocates runtime truth from `design/` to `.code-pact/state` + generated control snapshots: completed `design/phases/*.yaml` and retired `design/decisions/*.md` become **ephemeral** (hand-removable / `.gitignore`-able once snapshotted; missing *archived* docs tolerated, missing *active* docs fail closed). Demotes `PRUNED.md` to read-only backcompat (the v2.0 tombstone lives under `.code-pact/state`). **Supersedes** constitution "`design/` is the source of truth" and the move-only-non-gate-records rule below. Staged PRs, reader-side backcompat. **Itself retire-able** after v2.0 lands. |
