...
| — | [Archive-level compaction](archive-level-compaction-rfc.md) | **Proposed (design contract — no code authorized).** Closes the "moved the pile" gap: the per-item archive records (`state/archive/phases`/`event-packs`/`decisions/*.json`) are an INTERMEDIATE form that must fold into content-addressed **bundles** (loose∪bundle resolution, fail-closed binding, sharded so no bundle is itself unbounded, prune-capable). Generalises [event-pack compaction](event-pack-compaction-rfc.md) one level up so no level — loose docs, loose events, loose archive records — grows monotonically. Ships in reviewed layers (reader/binding → bundle writer → gated loose-record deletion). |
