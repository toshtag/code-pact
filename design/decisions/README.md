...
| — | [Decision record lifecycle](decision-lifecycle-rfc.md) | **Proposed.** Make a *shipped* decision record safely removable: status-aware ref integrity (a `done` task no longer hard-requires its `decision_refs` file on disk), a `decision prune` command with eligibility gates + an append-only `PRUNED.md` tombstone ledger, and a per-project `decision_retention` policy (`keep-full` / `compress-on-ship` / `prune-on-ship`). Plus the long-term record-of-truth model (CHANGELOG authored + rolling-archived; release notes generated; git is the backstop). |
