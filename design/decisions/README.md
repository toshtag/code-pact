...
| P45 | [Context-pack write contract hygiene](context-pack-write-contract-hygiene-rfc.md) | Routed `writeContextPack()` through `atomicWriteText` (it used a raw `writeFile`, breaking the published "every write is atomic" guarantee) and corrected the docs that blur `task context` (read-only — builds/returns the pack) with `task prepare` / `pack` (the writers). Patch-level (v1.29.1); no surface change. |
