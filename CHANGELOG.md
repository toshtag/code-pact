...
Contract hygiene. Context-pack writes are now atomic (matching the published "every write goes through `atomicWriteText`" guarantee), the docs no longer blur which command writes the pack, and the fresh-project `doctor` / agent-profile-path fixes from this cycle ship in the same patch. No new surface — all changes are advisory/internal or correct an existing contract.
