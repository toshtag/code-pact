...
- **Added versioned task-level review contracts.** A task can now declare the review boundary its work must survive *before* implementation starts. `review_contract` carries a `version`, a `mode` (`minimal` for low-risk docs / mechanical refactors, `boundary` for everything else), and — under `boundary` — a decision for all five review stages and all three platforms, each exactly once, plus at least one evidence entry. An `in_scope` stage needs a claim and refs; a `not_applicable` stage needs a rationale and no refs. A `required` platform needs an `integration` or `actual_platform` level and refs; a `not_required` platform needs a rationale and neither. The field is optional, so historical tasks and archived phases stay readable. Unknown keys are rejected on every object, one semantic validator backs `plan lint`, `task add`, `task lock`, and `task start`, and the contract is folded into the task-registration canonical JSON and digest. When present it is stored in the immutable contract lock, so any later edit to a stage, platform, claim, evidence entry, or ref surfaces as `TASK_CONTRACT_DRIFT` with `review_contract` in `changed_fields`. `plan lint` gains `TASK_REVIEW_CONTRACT_MISSING` (advisory — `affects_exit: false`, never fails `--strict`) and `TASK_REVIEW_CONTRACT_INVALID` (error). See [Review contracts](docs/review-contract.md).
- **Added `task add --review-contract-file <path>`.** Reads a `review_contract` fragment — the contract itself, YAML or JSON, not a whole task spec — and attaches it to the created task. Mutually exclusive with `--spec-file`, which carries its own contract; combining them is a `CONFIG_ERROR` rather than a silent precedence rule. A fragment whose shape or semantics do not hold is `TASK_REVIEW_CONTRACT_INVALID` (exit 2) with `data.issues[]` and **no** phase YAML write. `init --sample-phase` and `code-pact tutorial` now generate a worked example of each mode.
- **Added the `review_contract_policy` rollout switch.** `.code-pact/project.yaml` `review_contract_policy: advisory | required` decides whether a task that declares **no** contract may be locked. An absent field reads as `advisory`, so upgrading never makes an existing plan unlockable; the schema writes no default, so "never heard of review contracts" stays distinguishable from "decided not to require them". Under `required`, `task lock` and the `task start` auto-lock refuse with `TASK_REVIEW_CONTRACT_REQUIRED` (exit 2, `data.task_id` and `data.review_contract_policy`) and write **no lock file and no progress event**. A value outside the enum is a project schema error reported by `validate` / `doctor` and a `CONFIG_ERROR` on the lock path, never a silent fallback to `advisory`. `init` writes `required` into new projects. The refusal applies to new locks only — existing locks, archived phases, and done tasks are never rewritten or retro-validated.
- **Added staged verification.** `verify --stage focused | full` and a `.code-pact/project.yaml` `verification_policy` (`focused_command`, `max_full_attempts`, default `2`) let a project run a cheap focused gate before the full one. Focused results are recorded in a derived, gitignored, content-bound ledger under `.code-pact/cache/verification-runs/`; a second full gate for the same task and workspace digest is refused until a newer focused pass lands (`FULL_RETRY_REQUIRES_FOCUSED_PASS`), and the attempt budget is enforced (`FULL_VERIFICATION_BUDGET_EXCEEDED`). The path is fail-closed: an unprovable Git state is `VERIFICATION_STATE_UNAVAILABLE`, an untrusted ledger is `VERIFICATION_LEDGER_INVALID`, and requesting the focused stage without a policy is `FOCUSED_VERIFICATION_NOT_CONFIGURED`. Omitting `--stage` preserves the historical full-verification contract exactly.
...
- **Made `task finalize` / `phase reconcile` lossless on phase YAML.** The shared status writer parsed the file into the strict `Phase` model and serialized that model back, so keys the model does not carry — including a top-level `evidence:` — were silently discarded and modelled block scalars were re-wrapped, while the command still exited `ok: true`. The status write now locates the selected task's status in the original source and replaces only that scalar, preserving comments, unknown keys, formatting, and quote style, and it uses the original raw bytes as the atomic replacement expectation so a concurrent writer keeps its edit instead of being reverted.
- **Adopted flat argv as the verification-command contract.** The verification-scope producer emits one flat argv array per command, but the consumer declared a nested `[program, args]` pair, so the second element was a single string that spread into one argument per character. Every classifier command was unrunnable, and `task review-bundle` and `ci-parity` refused with a `VERIFICATION_FAILED` naming a command nobody wrote. The whole envelope is now schema-validated instead of cast, an empty program is rejected, legitimate single-character arguments are preserved, and validated argv is executed with `shell: false` while keeping the existing timeout and process-tree guarantees. Windows executable resolution gained a tested conservative boundary that refuses an unsafe batch shim rather than launching it.
...
- **Added lossless `--spec-file` task registration and locking.** `task add --spec-file` stores a strict task spec without dropping declared reads and writes; `task lock --spec-file` records its canonical registration proof. `task start` rejects later spec or contract drift before writing a started event.
...
- **Aligned task-id linting with existing trial task records.** `plan lint` accepts an optional single uppercase suffix such as `P80-T2C` while continuing to warn on lowercase, multi-letter, or arbitrary suffixes.
...
- **Made projection guidance conformance locale-safe.** Adapter conformance now accepts complete English or Japanese structural-projection guidance while rejecting incomplete mixed variants, and the public contract clarifies that `pack` remains an unbudgeted writer without deferred-context manifest materialization.
- **Completed structural-projection conformance output.** The public `adapter conformance` contract now documents the locale-safe projection check and its generator-version gate, while human-readable failures render nested locale-variant diagnostics as structured JSON instead of `[object Object]`.
...
- **Closed deferred-context readback races.** `task prepare` now returns the documented `CONTEXT_NOT_FOUND` exit-1 envelope when a competing cache mutation removes an artifact before readback, instead of escalating the condition to an internal error.
