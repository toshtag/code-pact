...
- **P43-T2 — `ADR_COMMITMENTS_EMPTY` plan-lint advisory (closes P43).** `plan lint --include-quality` now emits `ADR_COMMITMENTS_EMPTY` (`severity: warning`, `affects_exit: false` — never changes the exit code, including under `--strict`) when an **accepted** ADR that resolves a `requires_decision` task's decision gate records no implementation commitments — no `## Implementation commitments` section, or one with zero checkbox items. Scoped to accepted ADRs **referenced by a gated task** (via the shared resolver), so a historical ADR no task references never fires — keeping the advisory signal, not noise. One issue per ADR file (first referencing task wins); `file` is the ADR path with **no `path`** field (the subject is ADR content, not a plan-YAML field, matching the other ADR-centric advisories); `details.has_section` / `details.item_count` distinguish "no section" from "empty section". Registered in `KNOWN_CODES` + the plan diagnostic codes table; documented in `docs/cli-contract.md`, `docs/troubleshooting.md` (warning not blocker; the no-work anti-abuse note), and `docs/concepts/decision-gate.md`.
