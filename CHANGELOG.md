...
- **Hardened local loop memory.** Episode scans now reject oversized files at the bounded read boundary, require filename/content identity and canonical UTC timestamps, omit unused Evidence references from schema v1, distinguish write vs retention warnings, preflight prune batches, keep protected episodes within retention caps, redact more shell absolute-path forms, return stable memory JSON errors, and use Git's ignore semantics for doctor cache checks.
